...
isMooReadyForEdits:-isKBCompilerProcess(KB,Progress),!,write_ln(false).
isMooReadyForEdits:-write_ln(true).


isUncanonicalized(KB):-fail.	%TODO
isSourceNewerThenImage(KB):-fail.	%TODO
isKnowledgeBaseLoaded(KB,Ctx):-mooCache(_, _, _,_,KB,Ctx, _, _, _),!.
isKbUntransfered(KB):-not(isKnowledgeBaseLoaded(KB,_)).
isKbLoading(KB,Status):-isMooProcess(ID,loadKBfromSource(KB,GlobalContext)),!.

isKBCompilerProcess(KB,LastPercent):-isMooProcess(ID,canonicalizeMooKBHTML(KB,GlobalContext)),!,
			flag('$last_written_percent',LastPercent,LastPercent).

isKBCurrentlyInUse(KB,unloaded):-nonvar(KB),isKbUntransfered(KB),!,fail.
isKBCurrentlyInUse(KB,canonicalizing(Progress)):-isKBCompilerProcess(KB,Progress),!.
isKBCurrentlyInUse(KB,loading(Status)):-isKbLoading(KB,Status),!.
isKBCurrentlyInUse(KB,_):-!,fail.

invokeKBCompilerProcess(KB):-!.
invokeKBCompilerProcess(KB):-not(isUncanonicalized(KB)),!.
invokeKBCompilerProcess(KB):-
	mooProcessCreate(canonicalizeMooKBHTML(KB,GlobalContext),
	canonicalizeMooKBHTML(KB,GlobalContext),ID,[detatched(true)]),!.

% =================================================
% Divide Fact and Rules	(Mine logic and create explaination node)
% =================================================
testrcan:-recanonicalizeMooKB('PrologMOO').
...
expireTN(KB,TN):-
	logOnFailureIgnore(retractall(mooCache(Literal,_,KB,Ctx,TN))),  %Facts
	logOnFailureIgnore(retractall(mooCache(Literal,AnteLiteral,_,KB,Ctx,TN))).
...
changeSurfaceStatus(KB,TN,Before,After):-
	retract(mooCache(Surface,CAN,Flags,Vars,KB,Ctx,TN,Maintainer,Before)),
	assertz(mooCache(Surface,CAN,Flags,Vars,KB,Ctx,TN,Maintainer,After)),fail.
changeSurfaceStatus(KB,TN,Before,After):-!.
		


canonicalizeMooKBHTML(KB):-!.
canonicalizeMooKBHTML(KB):-
	tell('errors.html'),
	saveMooCache,
	logOnFailure(canonicalizeMooKB(KB)),
	saveMooCache,
	told.
canonicalizeMooKBHTML(KB):-!.
...
rct:-canonicalizeMooKBReal('PrologMOO').

canonicalizeMooKB(KB):-!.
canonicalizeMooKBReal(KB):-
	clearCanonicalizerWarnings(KB),
	writeDebug(starting(canonicalizeMooKB(KB))),
	flag(explaination_id,_,1),
	recanonicalizeTN(KB,TN),
	writeDebug(done(canonicalizeMooKB(KB))),!.
...
recanonicalizeTN(KB,TN):-
	flag('$sofar',_,1),
	countAssertions(mooCache(_,_,_,_,KB,Ctx,TN,_,_),Total),
	ifInteractive((writeFmt('\Compiling ~w surface clauses in ~w\n',[Total,KB]),writePercentAndTimeReset)),
	ignore((mooCache(Surface,CAN,Flags,Vars,KB,Ctx,TN,Maintainer,Result),  
	once((
	%	numbervars((Surface,CAN,Flags,Vars,KB,Ctx,TN,Maintainer,Result),'$VAR',0,_),!,
		canonicalizeClause(Surface,CAN,Flags,Vars,KB,Ctx,TN,Maintainer,Result,AssertList),
	...
	ifInteractive((
		...
		countAssertions(mooCache(_,_,KB,Ctx,TN),Facts),
		countAssertions(mooCache(_,_,_,KB,Ctx,TN),Rules),
		writeFmt('\n100% complete.  Examined ~w internal lemma structures for ~w. Facts:~w  Rules:~w  \n',[ExplainationID,KB,Facts,Rules]))),!.
...
% ================================================================
canonicalizeClause(Surface,true,Flags,Vars,KB,Ctx,TN,Maintainer,Result,[]):-!.
canonicalizeClause(Surface,not(true),Flags,Vars,KB,Ctx,TN,Maintainer,Result,[]):-!.
canonicalizeClause(Surface,Var,Flags,Vars,KB,Ctx,TN,Maintainer,Result,[]):-isSlot(Var),!.

% KB |= CAN1 & CAN2
canonicalizeClause(Surface,and(CAN1,CAN2),Flags,Vars,KB,Ctx,TN,Maintainer,Result,AssertionList):-!,
	(canonicalizeClause(Surface,CAN1,Flags,Vars,KB,Ctx,TN,Maintainer,Result,AssertList1)),
	(canonicalizeClause(Surface,CAN2,Flags,Vars,KB,Ctx,TN,Maintainer,Result,AssertList2)),!,
	append(AssertList1,AssertList2,AssertionList).     

canonicalizeClause(Surface,CAN,Flags,Vars,KB,Ctx,TN,Maintainer,Result,AssertList):-
	unnumbervars((Surface,CAN,Flags,Vars,KB,Ctx,TN,Maintainer,Result),
			   (USurface,UCAN,UFlags,UVars,UKB,UCtx,UTN,UMaintainer,UResult)),
	canonicalizeRule(USurface,UCAN,UFlags,UVars,UKB,UCtx,UTN,UMaintainer,UResult,AssertList),!.
...
canonicalizeRule(Surface,Rule,Flags,KRVars,KB,Ctx,TN,Maintainer,Result,Assertions):-!,
	...
	canonicalizeLemme(Surface,Rule,Cons,Ante,NewAnteR,Flags,KRVars,KB,Ctx,TN,Maintainer,Result,Assertions),!.
...
canonicalizeLemme(Surface,Rule,Cons,Ante,NewAnteR,Flags,KRVars,KB,Ctx,TN,Maintainer,Result,[]):-
...
canonicalizeLemme(Surface,Rule,Cons,Ante,NewAnteR,Flags,KRVars,KB,Ctx,TN,Maintainer,Result,Assertions):-
	...
	setMooOption(putAttributeStructures,Surface:Rule:CLID:Flags:KRVars:KB:Ctx:TN),!,
...
getAssertMatrix(ProtoType,(C:-true),KR,CLID,KRVars,KB,Ctx,TN,[mooCache(C,ExplainationID:KRVars:KR,KB,Ctx,TN)]):-
	flag(explaination_id,ExplainationID,ExplainationID+1).

getAssertMatrix(ProtoType,(C:-A),KR,CLID,KRVars,KB,Ctx,TN,[mooCache(C,A,ExplainationID:KRVars:KR,KB,Ctx,TN)]):-
	flag(explaination_id,ExplainationID,ExplainationID+1).
getAssertMatrix(ProtoType,C,KR,CLID,KRVars,KB,Ctx,TN,[mooCache(C,ExplainationID:KRVars:KR,KB,Ctx,TN)]):-
	flag(explaination_id,ExplainationID,ExplainationID+1).
...
ProtoType: putAttributeStructures(Surface,KRVars,Rule,KB,TN,CLID, ExplainationID,Anontate,Flags,IN_UnconstrainedFormua,OUT_UnconstrainedFormua)
...
Examples:
   ...
   | ?- putAttributeStructures(Surface,KRVars,Rule,KB,TN,CLID, ExplainationID,Anontate,Flags,('instance'(R,agent) :-'instance'(R,human)),S).
   R = _h99
   S = ('instance'(_h99,agent) :- 'instance'(_h99,human))
   
   | ?- putAttributeStructures(Surface,KRVars,Rule,KB,TN,CLID, ExplainationID,Anontate,Flags,('instance'(R,agent):-performs(R,task1)),S).
   R = _h95
   S = ('instance'(_h95,agent) :- 'domain'(performs,1,_h95)  ','  performs(_h95,task1))
   
   | ?- putAttributeStructures(Surface,KRVars,Rule,KB,TN,CLID, ExplainationID,Anontate,Flags,('instance'(R,agent):-performs(R,Act)),S).
   R = _h95
   Act = _h141
...
localCanonicalizerNotice(Type,Details):-
       getMooOption(putAttributeStructures,Surface:Rule:CLID:Flags:KRVars:KB:Ctx:TN),
       % writeq(Warning:Data),nl,
...
putAtomAttribute(KB,Anontate,Flags,Caller,ArgN,ArgIn,SlotStructure):-atom(ArgIn),	
	mooCache(KB,Anontate,putAtomAttributeProc(ArgIn,SlotStructure)),!.

*/
putAtomAttribute(KB,Anontate,Flags,Caller,ArgN,ArgIn,SlotStructure):-
	putAtomAttributeProc(KB,Anontate,Flags,Caller,ArgN,ArgIn,SlotStructure),!.
%	asserta(mooCache(KB,Anontate,putAtomAttributeProc(ArgIn,SlotStructure))),!.
...
% Any Name
putAtomAttributeProc(KB,Anontate,Flags,Caller,ArgN,ArgIn,SlotStructure):-
	deduceClassFromEntity(KB,Anontate,ArgIn,Class),nonvar(Class),!,
	getConstraintStructureForClass(ArgIn,Class,Slot),
...
putAtomAttributeProc(KB,Anontate,Flags,Caller,ArgN,ArgIn,SlotStructure):-
	ground(ArgIn),
	mooCache(KB,Anontate,classFromWrap(ArgIn,Class)),!,
	getConstraintStructureForClass(ArgIn,Class,Slot),
	applyAttributeStructureToEntity(Caller,ArgN,ArgIn,Slot,SlotStructure),!.
...
putAtomAttributeProc(KB,Anontate,Flags,Caller,ArgN,ArgIn,SlotedA):-
	...
	(ground(classFromWrap(ArgIn,Assumed)) ->asserta(mooCache(KB,Anontate,classFromWrap(ArgIn,Assumed))) ;true).
...
deduceClassFromEntity(KB,Anontate,Entity,Class):-
%	deduceTransitiveClosure_PartialOrderingRelation(KB,Anontate,Agent3,subclass,M,Class,SCExplaination),
	deduceSurface_gaf(instance(Entity,Class),_UAgent,KB,Explaination),not(Class='InheritableRelation'),!.
...
deduceSingleSubclassPathList(Entity,Origin,Destination,Path):-
		mooCache(KB,Anontate,deduceSingleSubclassPathList(Origin,Destination,Path)),!.

deduceSingleSubclassPathList(Entity,Origin,Destination,Path):-
	deduceSingleSubclassPathListProc(Entity,Origin,Destination,Path),
	asserta(mooCache(KB,Anontate,deduceSingleSubclassPathList(Origin,Destination,Path))).
...
deduceClassForDomainV(Pred:N,Class):-
		deduceSurfaceDomainVector(Arity,Pred,VectS,Agent,KB,Anontate,_),
		nth1(N,VectS,Class),!.
...
import_ado_cache(KBName,KB):-
	mooCache(_, surface, Axiom, Vars, KBName, Context, Tracking, User, Status),%trace,
	ado_cache_moo_assertKIF(Axiom,Tracking,Context,KB,KBName),
	fail.
...
ado_cache_moo_assertKIF(Axiom,Tracking,Context,KB,KBName):-
	moo_assertKIF(Axiom,Tracking,Context,KB,KBName),!.  % Puts it into MooHL % TODO
...
moo_assertKIF(Axiom):-				   
	getCurrentKnowledgeBase(KB),
	moo_assertKIF(Axiom,KB).

moo_assertKIF(Axiom,KB):-
	idGen(Tracking),
	moo_assertKIF(Axiom,Tracking,[],KBName).	

moo_assertKIF(Axiom,Context,KB):-
	idGen(Tracking),
	moo_assertKIF(Axiom,Tracking,[Context],KBName).	


moo_assertKIF(Axiom,Tracking,Context,KBName):-
	moo_assertKIF(Axiom,Tracking,Context,KBName,KBName).

% ===============================================================	
% moo_assertKIF(Axiom,Tracking,Context,KBObj,KBName)
% ===============================================================	

moo_assertKIF(Axiom,Tracking,Context,KB,KBName):-
	preprocessAxiom(Axiom,AxiomO),!,
	moo_assertKIF2(AxiomO,Tracking,Context,KB,KBName).
...
moo_assertKIF2(Axiom,Tracking,Context,KB,KBName):-
	moo_assertKIF_implicit(Axiom),!.
moo_assertKIF2(Axiom,Tracking,Context,KB,KBName):-
	moo_assertKIF_fast(Axiom,Tracking,Context,KB,KBName),!.
moo_assertKIF2(Axiom,Tracking,Context,KB,KBName):-
	moo_assertKIF_slow(Axiom,Tracking,Context,KB,KBName),!.
...
% ===============================================================      
moo_assertKIF_fast(KIF,Tracking,Context,KB,KBName):-
	moo_assertKIF_mapping(KIF,Tracking,Context,KB,Mapped),
...
moo_assertKIF_slow(KIF,Tracking,Context,KB,KBName):-
	moo_assertKIF_slowmapping(KIF,Tracking,Context,KB,Slowmapped),
...
subsumes(concepts,KB,CTX,C,D) :-
	...
	asserta((InHeadD :- call(G1))),
 	getRequest(KB,W1,C,aaa,Exp,InHeadC),
...
convertCTX(KB,[CTX1,Lits1],[dc(MOp,C)|L],WVL,[CTX3,Lits3],WVL3) :-
	...
	CTX2 = app(WPTerm:m(MOp,Agent),CTX1),
	getRequest(KB,CTX1,C,Agent,_Exp,Body),
...
genclass(KB,[W1,G1],concept(C),Agent,C,(Body,G1)) :-
	!,
	getRequest(KB,W1,C,Agent,_Exp,Body),
	!.
...
%       ???	
        defineEquivalentConceptname(KBName,CTX,CT,request,C),
        ...
        getRequest(KB,W1,C,X,_Exp,Goal),
...
undefconcept(KBName,CTX,CN,CT) :-
	...
	retractall(axiom(KB,CTX,defconcept(CTX,CN,CT))),
 	...
 	cmp_retractAllProlog(request(KB,CTX,CN,_CT,_PT,_)),
...
undefConcept(KB,CTX,CN) :-
	...
	retractall(conceptEqualSets(KB,system,CTX,_,_,AX)),
   	cmp_retractAllProlog(request(KB,CTX,not(CN),_CT,_PT,_)),
	fail,
	!.
...
cmp_retractAllProlog(request(KB,CTX,CN,_CT,_PT,_PT1)) :-
	request(KB,CTX,CN1,CT,PT,PT1),
	collect(PT,Liste),
	member(CN,Liste),
	retractall(request(KB,CTX,CN1,CT,PT,PT1)),
	fail.
cmp_retractAllProlog(request(KB,CTX,CN,_CT,_PT,_PT1)).
...
undefprimrole(KBName,CTX,RN,RT) :-
	...
	retractall(axiom(KB,CTX,defprimrole(CTX,RN,RT))),
 	retractall(kb_in(KB,_,rn(AX,_,_,_),_,_,_,_,_,_,proved(instance([],Name,_),_,_))),
 	cmp_retractAllProlog(request(KB,CTX,RN,_RT,_PT,_)),
	retractall(eq(KB,rn(AX,_,_,_),_,_,_,_,_,_)),
	retractall(eq(KB,rn(AX,_,_,_),_,_,_,_,_,_,_)),
	retractall(constraint(KB,rn(AX,_,_,lInR),_,_,_,_,_)),
	retractall(constraint(KB,rn(AX,_,_,lInR),_,_,_,_,_,_)),
...
delete_ind(KBName,CTX,X,C) :-
	...
	cmp_retractAllProlog(request(KB,CTX,_C,_X,_PT,_)),
	convertCTX(positive,KB,[[],true],CTX,[],[W1,G1],_),
...
moo_primconcept(CTX,Left) :-
	...
	!,
	getCurrentKnowledgeBase(KBName),
	moo_primconcept(KBName,CTX,Left),
	!.
...
moo_primconcept(CName1,[X|T]):-
	getCurrentKnowledgeBase(KBName),
	moo_primconcept(KBName,[],CName1,[X|T]),
	!.
...
moo_primconcept(A,CName1,[X|T]):-
	...
	getCurrentKnowledgeBase(KBName),
	!,
	moo_primconcept(KBName,A,CName1,[X|T]).
...
moo_primconcept(KBName,CTX,CName):-
	   knowledgeBase(KBName,KB,_),
	   (CTX = [] ; CTX = [_|_]),
...
/*-----------------------------------------------------------------------------
 * Der 4-stellige Konstruktor moo_primconcept(KBName,CTX,CName1,SpecListe) wird aktiviert 
 * mit expliziter Angabe von KnowledgeBase=KBName und modal context =CTX.
 *-----------------------------------------*/

moo_primconcept(KBName,CTX,CName1,[]) :-
	!.
moo_primconcept(KBName,CTX,CName1,[X|T]):-
	make_primconcept(KBName,CTX,CName1,X),
        moo_primconcept(KBName,CTX,CName1,T),
	!.
...
expand_role(KBName,CTX1,RTerm,RName1,CName1,CName2,CNameDef) :-
	RTerm = moo_primelemrole(KBName,CTX2,RName1, 'domain-range'(CName1,CName2,CNameDef)),
	!,
	append(CTX1,CTX2,CTX3),
	moo_primelemrole(KBName,CTX3,RName1, 'domain-range'(CName1,CName2,CNameDef)).
expand_role(KBName,CTX1,RTerm,RName1,CName1,CName2,CNameDef) :-
...
moo_defconcept(C1,SpecList):- 
	getCurrentKnowledgeBase(KBName),
	moo_defconcept(KBName,[],C1,SpecList),
	!.
...
moo_defconcept(A,C1,SpecList) :-
	...
	!,
	getCurrentKnowledgeBase(KBName),
	moo_defconcept(KBName,A,C1,SpecList),
	!.

/*-----------------------------------------------------------------------------
 * Der 4-stellige Konstruktor moo_defconcept(KBName,CTX,CName1,SpecListe) wird aktiviert 
 * mit expliziter Angabe von KnowledgeBase=KBName und modal context =CTX.
 *-----------------------------------------*/


moo_defconcept(KBName,CTX,C1,SpecList) :-
	moo_defconcept(KBName,CTX,C1,SpecList,[]).

moo_defconcept(KBName,CTX,C1,[],[]) :-
	!.
moo_defconcept(KBName,CTX,C1,[],CList) :-
	!,
	defconcept(KBName,CTX,C1,and(CList)),
	!.
moo_defconcept(KBName,CTX,C1,[supers(L)|SpecList],CList1) :-
	!,
	append(L,CList1,CList2),
	moo_defconcept(KBName,CTX,C1,SpecList,CList2).
moo_defconcept(KBName,CTX,C1,[Spec1|SpecList],CList1) :-
	make_defconcept(KBName,CTX,C1,Spec1,Concept),
	moo_defconcept(KBName,CTX,C1,SpecList,[Concept|CList1]).
moo_defconcept(KBName,CTX,C1,supers(L),[]) :-
	!,
...
moo_primelemrole(RName1, 'domain-range'(CName1,CName2,CNameDef)):-
	getCurrentKnowledgeBase(KBName),
	moo_primelemrole(KBName,[],RName1, 'domain-range'(CName1,CName2,CNameDef)).
...
moo_primelemrole(X,RName1, 'domain-range'(CName1,CName2,CNameDef)):-
	(X = [] ; X = [_|_]),
	getCurrentKnowledgeBase(KBName),
	moo_primelemrole(KBName,X,RName1, 'domain-range'(CName1,CName2,CNameDef)).

/*---------------------------------------------------------------------------
 * moo_primelemrole(KBName,CTX,RName1,domain-range(CName1,CName2,CNameDef))
 * definiert onee neue generelle Rolle RName1 mit CName1 als domain, CName2 
...
moo_defelemrole(RName1, restricts(RName2, range(CName1,CNameDef))):-
	getCurrentKnowledgeBase(KBName),
	moo_defelemrole(KBName,[],RName1,restricts(RName2,range(CName1,CNameDef))),
	!.
...
moo_defelemrole(X,RName1, restricts(RName2, range(CName1,CNameDef))):-
	...
	!,
	getCurrentKnowledgeBase(KBName),
	moo_defelemrole(KBName,X,RName1, restricts(RName2, range(CName1,CNameDef))).

/*----------------------------------------------------------------------------
 * moo_defelemrole(KBName,CTX,RName1,restricts(RName2,range(CName1,CNameDef)))
 * restrigniert die Rolle RName2 bzgl. des Rollenfuellers, auf CName1,
...
moo_disjoint(KBName,CTX,CName1,CName2):- 
...
% bilden und dann
% moo_ask(KB,CTX,(element(IC,NotConcept)))
% aufrufen
			 normalizeNot(not(Concept),NotConcept),
			 moo_ask(KB,CTX,(element(IC,NotConcept))),
			 cmp_nl,
...
moo_defelem(ICName1,SpecListe) :-
	getCurrentKnowledgeBase(KBName),
	moo_defelem(KBName,[],ICName1,SpecListe).
...
moo_defelem(A,ICName1,SpecList) :-
	...
	!,
	getCurrentKnowledgeBase(KBName),
	moo_defelem(KBName,A,ICName1,SpecList).
...
moo_defelem(KBName,CTX,ICName1,[]).
 
moo_defelem(KBName,CTX,ICName1,[X|T]):-
	make_defelem(KBName,CTX,ICName1,X),
	moo_defelem(KBName,CTX,ICName1,T). 
 ...
 *-----------------------------------------------------------------------*/
 ...
 moo_ask(KBName,CTX,(supers(CName1,CName2))) :-
...
/*------------------------------------------------------------------------------
 * moo_ask(KBName,CTX,(role(RName,CNameDom,CNameRan)))
 *
 ...
 *--------------------------------------------*/
 ...
 moo_ask(KBName,CTX,(role(RName,Subconcept,CNameRan))) :-
	  knowledgeBase(KBName,KB,_),
	moo_ask(KBName,CTX,(supers*(Subconcept,Superconcept))),
...
/*-----------------------------------------------------------------------------
 * moo_ask(KBName,CTX,(roleDef(RName,CNameDef)))
 *
...
/*----------------------------------------------------------------------------
 * moo_ask(KBName,CTX,(roleNr(RName,MinNr,MaxNr)))
 *
...
/*----------------------------------------------------------------------------
 * moo_ask(KBName,CTX,(roleDefNr(RName,DefNr)))
 *
 ...
 *
 * moo_ask(KBName,CTX,(element(ICName,CName)))
 *
...
moo_ask(KBName,CTX,(hl_attributes(CN,Hl_attribute,Value))) :-
	hl_attribute(concept,KBName,CTX,CN,[Hl_attribute,Value]).
moo_ask(KBName,CTX,(hl_attributes(CN,RN,Hl_attribute,Value))) :-
	hl_attribute(role,KBName,CTX,[CN,RN],[Hl_attribute,Value]).

moo_ask(KBName,CTX,(subrole(Subrole,Superrole))) :-
	getDirectFatherRoles(KBName,CTX,Subrole,RL),
	member(Superrole,RL).

moo_ask(KBName,CTX,(irole(R,X,Y))) :-
	knowledgeBase(KBName,KB,_),
	convertCTX(negative,KB,[[],true],CTX,[],[W1,G1],_),
...
%----------------------------------------------------------------------
% moo_ask(KBName,CTX,(allRoles(+CName,-Info)))
...
% bsp : moo_ask(kb,ctx,allRoles(golf,X))    X = ((has_part golf windshield 1 1 1)(consumes golf gasoline 1 1 1))
% 


moo_ask(KBName,CTX,(allRoles(CName,Info_list))) :-
	setofOrNil(Info,[RName,CNameRan,Info]^(moo_ask(KBName,CTX,(role(RName,CName,CNameRan))),
	 	 moo_ask(KBName,CTX,(allRoles(RName,CName,CNameRan,Info)))),Info_list).
	
moo_ask(KBName,CTX,(allRoles(RName,CName,CNameRan,[RName,CName,CNameRan,Min,Max,Defnr]))) :-
	moo_ask(KBName,CTX,(roleNr(RName,Min,Max))),
	moo_ask(KBName,CTX,(roleDefNr(RName,Defnr))),
	!.
moo_ask(KBName,CTX,(allRoles(RName,CName,CNameRan,[RName,CName,CNameRan]))) :-
	!.


%----------------------------------------------------------------------
% moo_fact(KBName,CTX,(all_roles(+CName,-Info)))
% Arguments: Cname ConceptName
%	     Info is a list consisting of lists with elements:
%               Rnames role-atom_codes
%	        Cnames domain 
...
%
% liefert alle rolen,domains,Minnr,Maxnr,Defnr die von oneem konzept ausgehen
% bsp : moo_fact(kb,ctx,allRoles(golf,X))    X = ((has_part golf windshield 1 1 1)(consumes golf gasoline 1 1 1))
% 


moo_fact(KBName,CTX,(allRoles(CName,Info_list))) :-
	setofOrNil(Info,[RName,CNameRan]^(moo_fact(KBName,CTX,(role(RName,CName,CNameRan))),
	 	 moo_fact(KBName,CTX,(allRoles(RName,CName,CNameRan,Info)))),Info_list).
	
moo_fact(KBName,CTX,(allRoles(RName,CName,CNameRan,[RName,CName,CNameRan,Min,Max,Defnr]))) :-
	moo_fact(KBName,CTX,(roleNr(RName,Min,Max))),
	moo_fact(KBName,CTX,(roleDefNr(RName,Defnr))),
	!.
moo_fact(KBName,CTX,(allRoles(RName,CName,CNameRan,[RName,CName,CNameRan]))) :-
	!.


%----------------------------------------------------------------------
% moo_ask(KBName,CTX,(direct_super_role(+-RName,+-CNameDom,+-CNameRan,+-SRName,+-SCNameDom,+-SCNameRan))) :-
...
moo_ask(KBName,CTX,(direct_super_role(RName,CNameDom,CNameRan,RName,SCNameDom,CNameRan))) :-
	isSlot(CNameDom),
...
moo_ask(KBName,CTX,(direct_super_role(RName,CNameDom,CNameRan,RName,SCNameDom,CNameRan))) :-
	moo_ask(KBName,CTX,(supers(CNameDom,SCNameDom))),
	!.
%	moo_ask(KBName,CTX,(role(SRName,SCNameDom,SCNameRan))),	
%	!.


%----------------------------------------------------------------------
% moo_ask(KBName,CTX,(risa(+-IRName,+-ICNameDom,+-ICNameRan,+-GRName,+-GCNameDom,+-GCNameRan))) :-
...
moo_ask(KBName,CTX,(risa(IRName,ICNameDom,ICNameRan,GRName,GCNameDom,GCNameRan))) :-
	...
	moo_ask(KBName,CTX,role(GRName,GCNameDom,GCNameRan)).

moo_ask(KBName,CTX,(risa(IRName,ICNameDom,ICNameRan,IRName,GCNameDom,ICNameRan))) :-
	knowledgeBase(KBName,KB,_),
	getConceptElement(KB,CTX,ICNameDom,GCNameDom),
	!.

%----------------------------------------------------------------------
% moo_ask(KBName,CTX,(rall(+-RName,+-CNameDom,+-CNameRan,CNameDef,+-MinNr,+-MaxNr,+-DefNr))) :-
% Arguments: Rname    RoleName
%	     CNameDom RoleDomain
%            CNameRan RoleRange 
...
moo_ask(KBName,CTX1,(rall(RName,CNameDom,CNameRan,CNameDef,MinNr,MaxNr,DefNr))) :-
	knowledgeBase(KBName,KB,_),
	hop_map(negate,CTX1,CTX2),
	hop_map(normalizeNot,CTX2,CTX3),
	convertCTX(negative,KB,[[],true],CTX3,[],[W1,G1],_),
...
moo_hl_attributes(CN,AList) :-
	getCurrentKnowledgeBase(KBName),
	moo_assert_hl_attributes(concept,KBName,[],CN,AList).
...
moo_hl_attributes(A1,CN,AList) :-
	...
	!,
	getCurrentKnowledgeBase(KBName),
	moo_assert_hl_attributes(concept,KBName,A1,CN,AList).
moo_hl_attributes(CN,R,AList) :-
	getCurrentKnowledgeBase(KBName),
	moo_assert_hl_attributes(role,KBName,[],[CN,R],AList).
moo_hl_attributes(A1,A2,CN,AList) :-
	knowledgeBase(A1,_,_),
	(A2 = [] ; A2 = [_|_]),
	!,
	moo_assert_hl_attributes(concept,A1,A2,CN,AList).
moo_hl_attributes(KBName,CN,RN,AList) :-
	moo_assert_Hl_attributes(role,KBName,[],[CN,RN],AList).
moo_hl_attributes(KBName,CTX,CN,RN,AList) :-	
	moo_assert_hl_attributes(role,KBName,CTX,[CN,RN],AList).

moo_assert_hl_attributes(Type,KBName,CTX,Spec,[]) :-
	!.
moo_assert_hl_attributes(Type,KBName,CTX,Spec,[Pair|AList]) :-
	assertz(hl_attribute(Type,KBName,CTX,Spec,Pair)),
	moo_assert_hl_attributes(Type,KBName,CTX,Spec,AList).

/*------------------------------------------------------------------------------
 * Die 2- bis 4-stellige Funktion moo_unprimconcept([KB],[CTX],CN,SpecListe) erwartet 
...
/*------------------------------------------------------------------------
 * moo_unprimconcept(KBName,CTX,CName1,SpecList)
 *
 *------------------------------------------*/
...
moo_unprimconcept(CName1,[X|T]) :-
			    getCurrentKnowledgeBase(KBName),
			    moo_unprimconcept(KBName,[],CName1,[X|T]),
			    !.
...
moo_unprimconcept(A,CName1,[X|T]) :-
			     ...
			     getCurrentKnowledgeBase(KBName),
			     !,
			     moo_unprimconcept(KBName,A,CName1,[X|T]).



moo_unprimconcept(KBName,CTX,CName1,[]) :- !.

moo_unprimconcept(KBName,CTX,CName1,[X|T]) :-
			     unmake_primconcept(KBName,CTX,CName1,X),
			     moo_unprimconcept(KBName,CTX,CName1,T),
			     !.
...
unexpand_role(KBName,CTX,RTerm,R1) :-
         RTerm = moo_unprimelemrole(KBName,CTX,R1, 'domain-range'(C1,C2,CNameDef)),
         !,
	 call(RTerm).
...
/**********************************************************************
 *
 * moo_undefconcept(KBName,CTX,CName1,SpecList)
 *
 *
 */
...
moo_undefconcept(CName1,SpecList) :-
         getCurrentKnowledgeBase(KBName),
	 moo_undefconcept(KBName,[],CName1,SpecList),
	 !.
...
moo_undefconcept(A,CName1,SpecList) :-
         ...
         getCurrentKnowledgeBase(KBName),
         !,
         moo_undefconcept(KBName,A,CName1,SpecList).
...
moo_undefconcept(KBName,CTX,CName1,[],[]) :- !.

moo_undefconcept(KBName,CTX,CName1,[],HList) :-
	   knowledgeBase(KBName,KB,_),
	   !,
...
moo_undefconcept(KBName,CTX,CName1,[supers(L)|SpecList],CL1) :-
	   !,
	   append(L,CL1,CL2),
	   moo_undefconcept(KBName,CTX,CName1,SpecList,CL2).

moo_undefconcept(KBName,CTX,CName1,[Spec1|SpecList],CL1) :-
           unmake_defconcept(KBName,CTX,CName1,Spec1,Concept),
           moo_undefconcept(KBName,CTX,CName1,SpecList,[Concept|CL1]).



moo_undefconcept(KBName,CTX,CName1,supers(L),[]) :-
           knowledgeBase(KBName,KB,_),
	   !,
...
/**********************************************************************
 *
 * moo_unprimelemrole(KBName,CTX,RName1, 'domain-range'(CName1,CName2,CNameDef)) *
 *
 */
...
moo_unprimelemrole(RName1, 'domain-range'(CName1,CName2,CNameDef)):-
           getCurrentKnowledgeBase(KBName),
	   moo_unprimelemrole(KBName,[],RName1, 'domain-range'(CName1,CName2,CNameDef)).
...
moo_unprimelemrole(X,RName1, 'domain-range'(CName1,CName2,CNameDef)):-
	   (X = [] ; X = [_|_]),
	   getCurrentKnowledgeBase(KBName),
	   moo_unprimelemrole(KBName,X,RName1, 'domain-range'(CName1,CName2,CNameDef)).
...
moo_undefelemrole(RName1, restricts(RName2, range(CName1,CNameDef))):-
        getCurrentKnowledgeBase(KBName),
	moo_undefelemrole(KBName,[],RName1, restricts(RName2, range(CName1,CNameDef))),
	!.
...
moo_undefelemrole(X,RName1, restricts(RName2, range(CName1,CNameDef))):-
	...
	!,
	getCurrentKnowledgeBase(KBName),
	moo_undefelemrole(KBName,X,RName1, restricts(RName2, range(CName1,CNameDef))),
	!.


moo_undefelemrole(KBName,CTX,RName1, restricts(RName2, range(CName1,CNameDef))) :-
        knowledgeBase(KBName,KB,_),
	!,
...
/**********************************************************************
 *
 * moo_undefelem(KBName,CTX,ICName1,[X|T])
 *
 */

moo_undefelem(ICName1,SpecListe) :-
	getCurrentKnowledgeBase(KBName),
	moo_undefelem(KBName,[],ICName1,SpecListe).
...
moo_undefelem(A,ICName1,SpecList) :-
	...
	!,
	getCurrentKnowledgeBase(KBName),
	moo_undefelem(KBName,A,ICName1,SpecList).


moo_undefelem(KBName,CTX,ICName1,[]) :-
	!.

moo_undefelem(KBName,CTX,ICName1,[X|T]):-
	unmake_defelem(KBName,CTX,ICName1,X),
        moo_undefelem(KBName,CTX,ICName1,T).


/**********************************************************************
 *
 * moo_fact(KBName,CTX,elementOf(X,C),P)
 *
 */

moo_fact(P1) :-
	completeParameter([P1],KBName,CTX,Request,Explaination),
	moo_fact(KBName,CTX,Request,Explaination).
moo_fact(P1,P2) :-
	completeParameter([P1,P2],KBName,CTX,Request,Explaination),
	moo_fact(KBName,CTX,Request,Explaination).
moo_fact(P1,P2,P3) :-
	completeParameter([P1,P2,P3],KBName,CTX,Request,Explaination),
	moo_fact(KBName,CTX,Request,Explaination).

moo_fact(KBName,CTX,element(X,C),Exp) :-
	retractall(hypothesis(_)),
 	knowledgeBase(KBName,KB,_),
 	convertCTX(negative,KB,[[],true],CTX,[],[W1,G1],_),
 	getNegatedConcept(C,C1),
 	getFactRequest(KB,W1,C,X,Exp,Goal),
 	call((call(G1), Goal)),
 	atomic(X),
	allowedAnswerConcept(KB,C).
moo_fact(KBName,CTX,(role(RName,CNameDom,CNameRan)),proved(fact,basedOn(tbox))) :-
	knowledgeBase(KBName,KB,_),
	!,
	getRoleDomain(KB,CTX,RName,CNameDom),
	getRoleRange(KB,CTX,RName,CNameRan).
moo_fact(KBName,CTX,(hl_attributes(CN,Hl_attribute,Value)),proved(fact,basedOn(tbox))) :-
	hl_attribute(concept,KBName,CTX,CN,[Hl_attribute,Value]).
moo_fact(KBName,CTX,(hl_attributes(CN,RN,Hl_attribute,Value)),proved(fact,basedOn(tbox))) :-
	hl_attribute(role,KBName,CTX,[CN,RN],[Hl_attribute,Value]).
moo_fact(KBName,CTX,irole(R,X,Y),Exp) :-
	retractall(hypothesis(_)),
	knowledgeBase(KBName,KB,_),
	convertCTX(negative,KB,[[],true],CTX,[],[W1,G1],_),
	getFactRequest(KB,W1,Y,R,X,Exp,Goal),
	call(G1), 
	call(Goal).
	
getFactRequest(KB,W1,C0,X,Exp,Goal) :-
...
%	constructMLHead(KB,_RN1,W1,C0,X,_HYPS,_D,_CALLS,PT,InHead),
	...
	!.
getFactRequest(KB,W1,Y,R,X,PT,Goal) :-
...
/***********************************************************************
 *
 * deduce(KBName,CTX,Request,Explaination)
 *
 */
...
/* deduce(KBName,CTX,elementOf(X,C),Exp) :-
	deduceMOOHL(KBName,CTX,elementOf(X,C),Exp).
*/
deduce(KBName,CTX,X,Exp) :-
	deduceMOOHL(KBName,CTX,X,Exp).
deduceMOOHL(KBName,CTX,elementOf(X,C),Exp) :-
	option(requestCaching,on),
	retractall(hypothesis(_)),
 	knowledgeBase(KBName,KB,_),
 	convertCTX(negative,KB,[[],true],CTX,[],[W1,G1],_),
	clause(request(KB,W1,C,X,Exp,Goal),_).
deduceMOOHL(KBName,CTX,elementOf(X,CT),Exp) :-
	retractall(hypothesis(_)),
	...
	constructMLMark(rn(_AN5,_RN5,_S5,_O5),W1,C1,X,_HYPS,_D1,InHead1),
 	getRequest(KB,W1,C,X,Exp,Goal),
	performRequest(X,G1,Goal),
	allowedAnswerConcept(KB,C),
% 	anlegen oneer clausel die instance undefconcept wieder geloescht wird...
 	setRequest(KB,W1,CT,X,Exp,Goal).
deduceMOOHL(KBName,CTX,roleFiller(X,R1,L,N),Exp) :-
	retractall(hypothesis(_)),
...
%	defineEquivalentRolename(KBName,CTX,R1,R),
	...
	convertCTX(negative,KB,[[],true],CTX,[],[W1,G1],_),
	call(G1),
	defineEquivalentRolename(KBName,CTX,R,request,R1),
	solveConstraint(KB,W1,(card,app((_FF:R1),X),'>=',_N1),(_N,L),
	                hyp([or([]),rl([]),fl([])]),ab(noAb),call([]),Exp),
	nonvar(X),
	length(L,N).
deduceMOOHL(KBName,CTX,roleConstraints(X,R1,['>=',N1,'=<',N2,L,N3]),Exp) :-
	retractall(hypothesis(_)),
	removeDefinitionOfRolename(requestrole),
%	defineEquivalentRolename(KBName,CTX,R1,R),
	R = R1,
...
defineEquivalentRolename(_,_,R,_,R) :-
	atomic(R),
	!.
defineEquivalentRolename(KBName,CTX,R,request,requestrole) :-
	defrole(KBName,CTX,requestrole,R),
	!.
...
defineEquivalentConceptname(_,_,C,_,C) :-
	atomic(C),
	!.
defineEquivalentConceptname(KBName,CTX,C,request,C1) :-
	C1 = requestconcept,
	defconcept(KBName,CTX,requestconcept,C),
	!.
...
setRequest(KB,W1,C,X,Exp,Goal) :-
	not(clause(request(KB,W1,C,X,Exp,_Goal),_)),
	assert(request(KB,W1,C,X,Exp,Goal)),
	!.
setRequest(KB,W1,C,X,Exp,Goal) :-
	!,
	fail.
...
getRequest(KB,CTX1,C0,X,C3) :-
	...
	clausesToSystemSyntax(C1,C2),
	splitGoal(C2,C3).
getRequest(KB,W1,C0,X,Exp,Goal) :-
	isSlot(C0),
	clause(conceptName(KB,_,_,C0),_),
...
/**********************************************************************
 *
 * completeParameter(+ParameterList,-KBName,-CTX,-Request,-Explaination)
 * takes a list of parameters ParameterList and instantiates the
 * variables KBName,CTX,Request,Explaination correctly.
 *
 */

completeParameter([Request],KBName,[],Request,_Explaination) :-
	getCurrentKnowledgeBase(KBName),
	!.
...
completeParameter([P1,P2],P1,_CTX,P2,_Explaination) :-
	...
	P1 \== [],
	!.
completeParameter([P1,P2],KBName,P1,P2,_Explaination) :-
	nonvar(P1),
	(P1 = [] ; P1 = [_|_]),
	!,
	getCurrentKnowledgeBase(KBName).
