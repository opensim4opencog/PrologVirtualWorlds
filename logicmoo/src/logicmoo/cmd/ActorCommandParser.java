...
public class ActorCommandParser implements IMooClient, IActorCommandParser {
	...
	if( cmd.equals("maxSeconds") && st.length==1 ) {
	    println( "the maxSeconds was " + (new Date()).toString());
	    return true;
	}
	...
	//if( cmd.startsWith(interp) ) 
	    return enactPrologChat(location,user, st[0] + " " + params);
	//else
	...
	enactProlog("serviceCycApiRequestSubP(\"" + subp + "\").");
    }
    public void enactCycLBot( String subp) {
	enactProlog("enactCycLBot(\"" + subp + "\").");
    }

    public boolean enactPrologChat(Object location, Object user, Object speech) {
	enactProlog("from(\"" + location + "\",\"" + user + "\",say(\"" + speech + "\")).");
	return true;
    }
