...
/**
    ...
    produceing an error and the system was never able to read it in,
...
fread(Reader,Body, Vars, NamedBody, NamedVars):-
  get(Reader,the(clause(Body,Vars,NamedBody,NamedVars))).
...
fwrite(O,T):-put(O,T).
fnl(O):-fput_codes(O,"\n").
fput_codes(O,Codes):-put(O,'$string'(Codes)).
...
% use lazy_head(LazyList,Head) and lazy_tail(LazyList,LazyTail) to make LazyList advance
% note that LazyTail is a new lazy list

% creates alazy list form an answer sourcelazy_findall(X,G,LazyList):-answer_source(X,G,S),source_lazy_list(S,LazyList).
% creates a lazy list from a listlazy_list(List,LazyList):-
  list_source(List,S),
  source_lazy_list(S,LazyList).  
% returns a head/tail pair of a non-empty lazy listlazy_decons(XXs,X,Xs):-neq(XXs,[]),lazy_head(XXs,X),lazy_tail(XXs,Xs).

% explores a lazy list in a way compatible with backtracking
% allows multiple 'consumers' to access the list, end ensures that% the lazy list advances prograssively and consistently
lazy_element_of(XXs,X):-
  lazy_decons(XXs,A,Xs),  lazy_select_from(Xs,A,X).
% backtracks over the lazy list
lazy_select_from(_,A,A).
lazy_select_from(LXs,_,X):-lazy_element_of(LXs,X).
