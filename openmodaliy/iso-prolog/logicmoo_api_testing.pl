...
invokeTestSystem(Options):-
        ...
        setMooOption(Agent,opt_theory='LogicMOO'),
        getMooOption(Agent,asid=_,AID),
        getMooOption(Agent,user='Web',Agent),
        getMooOption(Agent,interp='kif',Interp),
        getMooOption(Agent,filename='axioms.tq',FileName),
        testTQFileWriteHTML(FileName,Agent,TimeRequired,Result,Title,Mods).
...
invokeTestSystem(Options):-memberchk(submit='Run TQs',Options),!,make,
        ...
        getMooOption(Agent,user='Web',Agent),
        getMooOption(Agent,interp='kif',Interp),
        getMooOption(Agent,tqDir='soundness',TQDIR),
        servantProcessCreate(ProcessName,run_tq_thread(TQDIR),Id,[]),!,
...
invokeTestSystem(Options):-memberchk(cmd='tqpage',Options),!,make,
        ...
        say('<table border=0>',[]),
        say('<th><b>View Test Source</b></th><th><b>Run Test</b></th><th></th><th><b>View Test Source</b></th><th><b>Run Test</b></th>',[]),
...
batch(Start,FileName,INDIRRELATE,Dest,OUTDIR):- !,
         ...
         Elapsed is TS - Start,
          ...
          say(Dest,'<BR><TABLE><TR><TD  width=250>Test Source</TD><TD width=100>Test Result</TD><TD width=192>Request Time</TD><TD width=192>Time Total</TD></TR>',[])
         ...
         getCputime(TE),
   (tq_ignored -> true ; ((
         (retract(oddeven) -> say(Dest,'<tr bgcolor=#CCCCEE> ',[]) ; (say(Dest,'<tr bgcolor=#FFFFFF> ',[]),assert(oddeven))),
         TSE is TE - TS,
         ...
         say(Dest,'<TD><a href="~w">~w&nbsp;</a></TD><TD><a href="~w"><font color=~w>~w</font></a></TD><TD>~w</TD><TD>~w secs </TD></TR> \n',[FileBaseName,Title,BaseResultFile,DISP,Result,CPUDISP,TSE])
        ))
...
summerize_dest(Dest,Comment,NEWDIR,Elapsed):-
         ...
         say(Dest,'<UL><Strong>Succesfully <Font color=green>Passed</Font> ~w and <Font color=orange>Failed</Font> ~w but <Font color=blue>Skipped</Font> ~w ',[TotalPassed,TotalFailed,Skipped]),
         say(Dest,'Test Questions (Total Counted ~w) giving us <font size="+1" color=purple>Passed ~w%</font> with <font size="+1" color=green>Absolutely ~w% </font> in ~w seconds.&nbsp;&nbsp;<BR>\n</Strong></UL>\n',[TotalRan,Percent,PercentComplete,Elapsed])
              ),_,true).
...
write_file_list_0(TQDIR,T,TQOutputDir):-
        ...
        say('<td><A href="belief_test/~w/~w">~w</A></td><td><A href="tests.jsp?cmd=runtest&filename=~w"><font color=green><b>Run Now</b></font></A></td>\n',[TQDIR,BaseName,BaseName,TO]).
...
links_to_available_tqs0(P):-
        say('<tr><td><font size=+1>~w<font></td>',[P]),
        concat_atom(['../../belief_test/',P],Dir),
        (running_tq(_) ->
                (say('<td> waiting </td>',[]),
                        (running_tq(P) ->
                                (
                                say('<td><A href="belief_test/~w/"><font color=green><i>Currenty Running</i></font></A></td>',[P,P])
                                )
                                ;
                                say('<td> waiting </td>',[])
                        )
                ...
                ;
                (
                say('<td> <A href="tests.jsp?cmd=tqpage&tqDir=~w">Live</A></td>', [P]),
                say('<td><A href="tests.jsp?submit=Run TQs&tqDir=~w">Batch Process</a></td>',[P,P])
                )
        ),
        (exists_directory(Dir) ->
                say('<td><P><A href="belief_test/~w/">View</a></td>',[P])
                ;
                say('<td>&nbsp;</td>')
        ),
        say('</tr>',[]).
...
run_batch_file_list(Type,Dest,TQS,TQOutputDir):-!,
         ...
         say(Dest,'<UL><H2>Moo Belief Engine Test Question Results</H2><BR>\n<H3>Ran at: <B>~w</B>\n<P></H3><BR>',[DateTime]),
         say(Dest,'<p><font color=green>Passed</font> = Definately and Deterministicly Passed<br><font color=green>Passed_Likey</font> = Passed but was not completely verified<br><font color=purple>Incomplete</font> = Passed but answers did notmatch verbatim (like decimal precision)<br><font color=green>Passed_Soundness</font> = Passed a soundness test due to unproven result<br><font color="#FF9900">Failed_Incomplete</font> = Failed due to lack of Explaination<br><font color="#0000FF">Syntax_Errors</font> = Test could not be accurately ran</p><p>' ,[]),
        % say(Dest,'<TABLE><TR><TD  width=250>Test Source</TD><TD width=100>Test Result</TD><TD width=192>Request Time</TD><TD width=192>Time Total</TD></TR>',[]),
         flush_output(Dest),
         run_batch_file_list1(Type,Dest,TQS,TQOutputDir),!.

run_batch_file_list1(Type,Dest,[],TQOutputDir):-!,say(Dest,'</TABLE>\n',_),!.
         ...
         (   % If/Then/Else (Make block Of TQs)
             (last_tq(LTQ),atom_codes(LTQ,[A,B,C|_]),not(atom_codes(FileBaseName,[_,_,C|_])))
                    ->
                          ...
                          say(Dest,'<BR><TABLE><TR><TD  width=250>Test Source</TD><TD width=100>Test Result</TD><TD width=192>Request Time</TD><TD width=192>Predicates Used</TD></TR>',[])
                          ))
                  ; true
          ),
         ...
         TotalTime is TimeElapsed - TimeStart,
          (tq_ignored -> true ;
                 ((
                         ...
                         say(Dest,'<TD><a href="~w">~w&nbsp;</a></TD><TD><a href="~w"><font color=~w>~w</font></a></TD><TD>~w</TD><TD>~w</TD></TR> \n',[FileBaseName,Title,BaseResultFile,ResultColor,Result,ColorTime,Mods])
                  ))
           ),
        retractall(last_tq(_)),
        assert(last_tq(FileBaseName)),
...
% ========================================================================================
% Run Batch File "FileName" to HTML Output "ResultFilename"  as "Agent" and return "TimeRequired","Result" and "Title"
% ========================================================================================
:-dynamic(request_time/1).


testTQFileWriteHTML(FileName,Agent,TimeRequired,Result,Title,Mods):-isMooOption(Agent,debug_tests,true),!,
         file_base_name(FileName,FileBaseName),
        setMooOption(Agent,client,html),!,
         getPrettyDateTime(DateTime),
         say('<HTML><HEAD><TITLE>~w</TITLE></HEAD><BODY>Mode: Belief-201<BR>Version: SWI-Prolog 4.0.10<P>Filename:<A href="~w">~w</A>\n\nRan: <B>~w</B>\n<pre>',
         [FileBaseName,FileBaseName,FileBaseName,DateTime]),
        setMooOption(Agent,(opt_timeout=120)), %2 minutes
        setMooOption(Agent,(opt_debug=on)),
        testTQFileWriteHTML0(FileName,Agent,TimeRequired,Result,Title,Mods),
         say('</pre></BODY><HTML>\n',[]).
...
testTQFileWriteHTML(FileName,Agent,TimeRequired,Result,Title,Mods):-
        ...
        setMooOption(Agent,client,html),!,
         ...
         say('<HTML><HEAD><TITLE>~w</TITLE></HEAD><BODY>Mode: Belief-201<BR>Version: SWI-Prolog 4.0.10<P>Filename:<A href="~w">~w</A>\n\nRan: <B>~w</B>\n<pre>',
         [FileBaseName,FileBaseName,FileBaseName,DateTime]),
        setMooOption(Agent,(opt_timeout=600)), %10 minutes
        setMooOption(Agent,(opt_debug=off)),

        testTQFileWriteHTML0(FileName,Agent,TimeRequired1,Result1,Title1,Mods1),
        ...
        setMooOption(Agent,(opt_timeout=45)), % 45 seconds
        setMooOption(Agent,(opt_debug=on)),
                testTQFileWriteHTML0(FileName,Agent,TimeRequired,Result,Title,Mods)
                )))),
         say('</pre></BODY><HTML>\n',[]).
...
testTQFileWriteHTML0(FileName,Agent,TimeRequired,Result,Title,Mods):- make,
        ...
        %say(user_error,'~q.\n',[Dir + FileBaseName = FileName]),
         ...
         repeat,
                setMooOption(Agent,client,html),
                 once(source_from_stream(INPUT,Chars,Formula,Vars)),nonvar(Formula),
                  logOnFailure(isMooOption(Agent,opt_ctx_assert=Ctx)),
                  logOnFailure(isMooOption(Agent,opt_theory=Context)),
                  %logOnFailure(once(toMarkup(html,formula(Formula),Vars,Out))),say('<hr>~w<br>',[Out]),
                   logOnFailure(once(invokeOperation(verbose,Formula,Ctx,Tracking,Context,Agent,Vars))),
         ...
         say('<hr><P><P>CPUTIME = <B>~w</b>',[TimeRequired]),
         color_result(Result,Color),
         say('<br>RESULT = <B><font color=~w>~w</font></b>',[Color,Result])
         )).
...
show_relations:-
        mooCache(R, surface, Axiom, Vars, ContextName, Context, Tracking, Agent, Status),
        show_relations(R),fail.

show_relations:-
        mooCache(R, Axiom, Vars, ContextName, Context, Tracking, Agent, Status),
        show_relations(R),fail.

show_relations:-
        mooCache(R, Axiom, Vars, ContextName, Context, Tracking, Agent),
        show_relations(R),fail.
...
/* Assertion
interfaceAssert(+KIFCharsIn,+Ctx,+Context,+Agent).
KIFCharsIn = "(instance virginia Computer)"
Ctx = 'Computers and Things'
Context = 'LogicMOO' currently must be set to 'LogicMOO'
Agent = 'dmiles@users.sourceforge.net'

*/
        % interfaceAssert(+KIFCharsIn,+Ctx,+Context,+Agent).
test_u_assert:-interfaceAssert("(instance hardwareType Relation)",'test','LogicMOO','tester') .
test_u_assert1:-interfaceAssert("(instance hardwareType Object)",'test','LogicMOO','tester') .


/*
Now the predicates that you most want
Invokes a request
interfaceRequest(+KIFCharsIn,+Ctx,+Context,+Agent,-UResultsSoFar,-Result,-Explaination,-Status).

Input variables:
KIFCharsIn = "(instance ?What Computer)"
Ctx = 'Computers and Things'  % Any legal Atom that helps you book keep
Context = 'LogicMOO' currently must be set to 'LogicMOO'
Agent = 'dmiles@users.sourceforge.net'  There are times when error messages about redundant assertions happen.. and for bookkeeping all assertions have an author atom.. perhaps you can even use the source file name it came from.
...
*/
                ...
                % interfaceRequest(+KIFCharsIn,+Ctx,+Context,+Agent,-UResultsSoFar,-Result,-Explaination,-Status).
test_u_request:-interfaceRequest("(hardwareType ?W ?U)",'test','LogicMOO','tester',U,R,P),
         writeObject(getPrologVars(R),R),writeObject(explaination(P),R),fail.

test_u_request2:-
         interfaceRequest("(hardwareType ?W ?U)",'test','LogicMOO','tester',U,R,P),
         not((U=(-1))),
