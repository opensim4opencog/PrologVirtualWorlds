...
Assertion: is any say in KIF/STANDARD/Prolog
...
% =====================================================================================
%  agentInsert(Insert_chars,Ctx,TN,Context,Agent)
% =====================================================================================
...
agentInsert(Insert_chars,Ctx,TN,Context,Agent):-!,   
            ...
            invokeInsert([untrusted,canonicalize],surface,Pterm,Ctx,TN,Context,Vars,Agent).
...
getTruthCheckResultsInsert(Driver,Surface,CAN,Flags,Ctx,TN,Context,Vars,Maintainer,Result):-
	isMooOption(Agent,opt_deduce_domains=off),       
...
% =====================================================================================
%  retract(Retract_chars,Ctx,TN,Context,Agent)
% =====================================================================================

bp_retract(Retraction_Chars,Context,TN,L,Agent):-
      (((idGen(TN),TrackingAtom=(TN)) ; TrackingAtom=TN)),!,
      setMooOption(Agent,OptionList),
      write_response_begin,!, %%true,
      tell_retract_parse_chars(Retraction_Chars,Formula,Vars),
      %%ignore(once(retract(Retraction_Chars,_Ctx,TrackingAtom,Context,Agent))),
      retract_odbc(Formula,Ctx,TrackingAtom,Context,Vars,Agent),
      write_response_end.
...
retract(Retract_chars,Ctx,TN,Context,Agent):-!,   
            ...
            moo_invoke(retract,forall,surface,Pterm,Ctx,TN,Context,Vars,Agent).
       */
