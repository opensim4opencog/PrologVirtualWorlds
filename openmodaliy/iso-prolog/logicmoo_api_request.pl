...
runServerThread(Port,Type) :-
	concat_atom([Type,' World Server'],Name),
        servantProcessCreate(kill,Name,networkServer(Port,Type),_,[local(250),global(500),trail(500),argument(125)]).
...
acceptNetworkClient(Sock,Type):- prolog_system(yap),!,
	socket_accept(Sock, Loc, ClientSocket),
	 ...
	 socket_buffering(Sock,write,_,unbuf),
		...
		cleanOldProcesses,
		servantProcessCreate(killable,Name,serviceAcceptedClient(Loc,ClientSocket,Type),_,[detatch(true),local(10000),global(20000),trail(20000),argument(5000)]),!.
...
packetFromCC(Channel,Agent,packet('world_0', 0, Agent, Serialno, S1970)):-
	 get_time(S1970),getLocalSerial(Serialno),
	 getActiveForCC(Channel,Agent,Agent).
