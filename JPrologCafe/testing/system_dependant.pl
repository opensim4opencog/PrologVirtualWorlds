...
:- use_module(engine(loading_code)).
...
% Threads 
% ========================================================================================
prolog_thread_create(Goal,Id,Options):-Goal. %thread_create(Goal,Id,Options).
prolog_current_thread(Id,Status):-fail. %current_thread(Id,Status).
prolog_thread_exit(Goal):-!. %thread_at_exit(Goal).
prolog_thread_self(Id):-Id=main. %thread_self(Id).
prolog_thread_at_exit(Goal):-!. %thread_at_exit(Id,Goal).
prolog_thread_signal(Id,X):-!. %thread_join(Id,X).
...
writeSavedPrompt:- \+ (isConsoleOverwritten),!.
...
writeErrMsg(Out,E):-writeFmtFlushed(Out,'<prolog:error>~s</prolog:error>\n',[S]),!.
writeErrMsg(Out,E,Goal):-writeFmtFlushed(Out,'<prolog:error>goal "~q" ~s</prolog:error>\n',[Goal,S]),!.
/*
...
numbervars(X):-!.
%get_time(T),convert_time(T,A,B,C,D,E,F,G),!,numbervars(X,'$VAR',G,_).
unnumbervars(X,X):-!. %term_to_atom(X,A),atom_to_term(A,Y,_).
...
moduleEnsureLoaded(X):-!,
	...
	use_module(X),_,(catch((atom_concat('mod/',X,Y),
        use_module(Y)),E,format(user_error,';; file find error ~q ~q',[X,E])))).
*/
...
processBootstrap:-
   ...
   moduleEnsureLoaded(Filename),fail.
