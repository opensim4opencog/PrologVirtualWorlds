...
go(N):-
	day_of_week(1993,4,9,_Day),
	% write('April 9, 1993 is: '),
	% write(Day), nl,
	measure(full_loop(N,10000),empty_loop(N,10000),Utime,Gtime,System),
	nl, write(bench(cal,N,Utime,Gtime,System)),write('.'),nl.
...
leap_year(Year,DOW0,_,DayOfWeek):-
	Tmp is Year mod 4, not(0 == Tmp),
	!,
	dow(DOW0,DayOfWeek).

leap_year(Year,DOW0,LeapC,DayOfWeek):-
    Tmp is Year mod 100, not(Tmp == 0),
	!,
	DOW is DOW0-LeapC,
	dow(DOW,DayOfWeek).

leap_year(Year,DOW0,_,DayOfWeek):-
	Tmp is Year mod 400, not(0 == Tmp),
	!,
	dow(DOW0,DayOfWeek).
