  ...
  public Predicate exec(Prolog engine) {
    ...
    */
    ...
    if (isAtomTerm(a1) || isNumber(a1) || isJavaObject(a1) || isClosure(a1)) {
      ...
      if (!unify(a2, makeList(sym, t))) return this.fail(engine);
    } else if (isVariable(a1)) {
      a2 = deref(a2);
      if (isVariable(a2)) throw new PInstantiationException(this, 2);
      if (prologEquals(a2, PRED_$univ_2.SYM_NIL)) throw new IllegalDomainException(this, 2, "non_empty_list", a2);
      if (!isListTerm(a2)) throw new IllegalTypeException(this, 2, "list", a2);
      final Object head = deref(first((Object) a2));
      Object tail = deref(rest((Object) a2));
      if (isVariable(head)) throw new PInstantiationException(this, 2);
      if (prologEquals(tail, PRED_$univ_2.SYM_NIL)) {
        if (isAtomTerm(head) || isNumber(head) || isJavaObject(head) || isClosure(head)) {
          if (!unify(a1, head)) return this.fail(engine);
          return this.cont;
        }
        throw new IllegalTypeException(this, 2, "atomic", head);
      }
      if (!isAtomTerm(head)) throw new IllegalTypeException(this, 2, "atom", head);
      for (Object x = tail; !prologEquals(x, PRED_$univ_2.SYM_NIL); x = deref(rest(x))) {
        if (isVariable(x)) throw new PInstantiationException(this, 2);
        if (!isListTerm(x)) throw new IllegalTypeException(this, 2, "list", a2);
      }
      final int n = listLength((Object) a2) - 1;
      final/*SymbolTerm*/Object sym = makeAtom(nameUQ(head), n);
      final Object[] args = new Object[n];
      for (int i = 0; i < n; i++) {
        args[i] = deref(first((Object) tail));
