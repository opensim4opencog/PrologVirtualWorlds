...
'$on_exception'(Catch, _Goal, Recovery) :-
        '$get_exception'(Ball),
        Ball \== '$no_ball',
	'$catch_and_throw'(Ball, Catch, Recovery).

'$catch_and_throw'(Ball, Ball, Recovery) :-  !, % unify succeeds
        '$set_exception'('$no_ball'),
	call(Recovery).
'$catch_and_throw'(Ball, _, _) :-  % unify fails!
...
%  The merge/3 are based on the DEC-10 Prolog Library:
%    File   : SORTS.PL
%    Author : R.A.O'Keefe

%sort(R, S) :- var(R), !, illarg(var, sort(R,S), 1).
%sort(R, S) :- '$sort'(R, S0, 0, <), !, S = S0.
%sort(R, S) :- illarg(type(list), sort(R,S), 1).

%keysort(R, S) :- var(R), !, illarg(var, keysort(R,S), 1).
%keysort(R, S) :- '$sort'(R, S0, 1, =<), !, S = S0.
%keysort(R, S) :- illarg(type('list of pair'), keysort(R,S), 1).

merge(A, B, M) :- var(A), !, illarg(var, merge(A,B,M), 1).
merge(A, B, M) :- var(B), !, illarg(var, merge(A,B,M), 2).
merge(A, B, M) :- '$merge'(A, B, M0, 0, =<), M = M0.

'$sort'([], [], _, _).
'$sort'([X], [X], _, _).
'$sort'([X,Y|L], Sorted, Key, Order) :-
	'$halve'(L, [Y|L], Front, Back),
	'$sort'([X|Front], F, Key, Order),
	'$sort'(Back, B, Key, Order),
	'$merge'(F, B, Sorted, Key, Order).

'$halve'([_,_|Count], [H|T], [H|F], B) :- !, '$halve'(Count, T, F, B).
'$halve'(_, B, [], B).

'$merge'([H1|T1], [H2|T2], [Hm|Tm], Key, Order) :- !,
	'$compare'(Key, Order, H1, H2, R),
	(   R = (<), !, Hm = H1, '$merge'(T1, [H2|T2], Tm, Key, Order)
	;   R = (>), !, Hm = H2, '$merge'([H1|T1], T2, Tm, Key, Order)
	;   R = (=), !, Hm = H1, '$merge'(T1, T2, Tm, Key, Order)
	).
'$merge'([], L, L, _, _) :- !.
'$merge'(L, [], L, _, _).
...
expand_term(DCG, Cl) :- '$dcg_expansion'(DCG, X), !, X = Cl.
expand_term(X, X).

'$dcg_expansion'(DCG, Cl) :- var(DCG), !, DCG = Cl.
'$dcg_expansion'(M:DCG, M:Cl) :- !, '$dcg_expansion'(DCG, Cl).
'$dcg_expansion'((Head --> B), (H1 :- B1, B2)) :-
	nonvar(Head),
	Head = (H,List), 
	!,
...
'$dcg_translate_dcg'(X, X1, S, S0, S) :- var(X), !,
	'$dcg_translate_dcg_atom'(X, X1, S0, S).
'$dcg_translate_dcg'(M:X, M:Y, Tail, S0, S) :- !,
	'$dcg_translate_dcg'(X, Y, Tail, S0, S).
...
'$dcg_translate_dcg_atom'(X, X1, S0, S) :- var(X), !,
	X1 = phrase(X,S0,S).
'$dcg_translate_dcg_atom'(M:X, M:X1, S0, S) :- !,
	'$dcg_translate_dcg_atom'(X, X1, S0, S).
'$dcg_translate_dcg_atom'(X, X1, S0, S) :-
	functor(X, F, A),
	A1 is A+1,
