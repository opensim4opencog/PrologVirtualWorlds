...
write_java0(get_con(_,Xi,Xj), In, Out) :- !,
	...
	tab(Out, 12), 
	write(Out, 'if (! '), 
	write_reg(Xj, Out), write(Out, '.prologEquals('), write_reg(Xi, Out), 
	write(Out, '))'), nl(Out),
	tab(Out, 16),
 	write(Out, 'return fail(engine);'), nl(Out),
	% write mode
...
write_java0(get_str(_F/A,Xi,Xj), In, Out) :- !,
	...
	write(Out, '.prologEquals(('), write_reg(Xj, Out),
...
write_inline0('$cut'(X), _, Out)       :- !,
	...
	write(Out, '} else {'), nl(Out),
	tab(Out, 12),
	write(Out, 'engine.cut(intValue( '), write_reg(X, Out), 
	write(Out, '));'), nl(Out),
	tab(Out, 8),
	write(Out, '}'), nl(Out).
...
write_inline0(java(X,Y), _, Out) :- !, 
	...
	EXP = #('makeAtom'(@(getName(@(getClass(@(object(cast('JavaObjectTerm',X))))))))),
