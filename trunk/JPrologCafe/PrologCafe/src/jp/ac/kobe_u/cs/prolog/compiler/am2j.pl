/*****************************************************************
Time-stamp: <2008-06-23 21:30:49 banbara>

NAME
       am2j: Translating WAM-based Intermediate Code into Java that is optimized to run in CycKB
...
pl2j(Pre):-
   atom_concat(Pre,'.pl',PL),
   atom_concat(Pre,'.am',AM),
   pl2am([PL,AM,[ed , ac , ie , rc , idx , clo ]]),
   am2j([AM]).
...
write_java0(info([FA,File|_]), _, Out) :- !,
	...
	write(Out, '/**'), nl(Out),
        ...
        write(Out, ' @author Douglas R. Miles (dmiles@users.sourceforge.net) for Term(s) '), nl(Out),
        write(Out, ' @version 1.0-dmiles'), nl(Out),
        write(Out, '*/'), nl(Out).
...
write_java0(execute(cont), _, Out) :- !,
	tab(Out, 8),
	write(Out, 'return exit(engine,cont);'), nl(Out).
write_java0(execute(BinG), _, Out) :- !,
	...
	G =.. [F|Args],
	tab(Out, 8),
	write(Out, 'return exit(engine, new '),
	(nonvar(P) -> write_package(P, Out), write(Out, '.') ; true),
	write_class_name(F/A, Out),
	write(Out, '('),
	write_reg_args(Args, Out),
...
write_java0(new_hash(Tag,I), _, Out) :- !,
	tab(Out, 4),
	write(Out, 'static HashtableOfTerm<Predicate> '),
	(Tag == int -> write(Out, 'Int') ; write(Out, Tag)),
	write(Out, ' = new HashtableOfTerm<Predicate>('),
	write(Out, I),
	write(Out, ');'), nl(Out).
...
write_java0(put_var(X), _, Out) :- !,
	tab(Out, 8),
	write_reg(X, Out), 
	write(Out, ' = engine.makeVariable(this);'), nl(Out).
write_java0(put_int(I,X), _, Out) :- !,
	tab(Out, 4),
	write(Out, 'static IntegerTerm '),
	write_reg(X, Out), 
	write(Out, ' = makeInteger('),
...
write_java0(put_float(F,X), _, Out) :- !,
	...
	write_reg(X, Out),
	write(Out, ' = makeDouble('),
	write(Out, F),
	write(Out, ');'), nl(Out).
write_java0(put_con(C,X), _, Out) :- !,
	tab(Out, 4),
	write(Out, 'static /*Symbol*/Term '),
	write_reg(X, Out),
	write(Out, ' = makeAtom("'),
	(C = F/A -> 
...
write_java0(put_list(Xi,Xj,Xk), _, Out) :- !,
	(Xk = s(_) -> 
	    tab(Out, 4), write(Out, 'static /*List*/Term ')
	    ;
	    tab(Out, 8)
	),
	write_reg(Xk, Out),
...
write_java0(put_str(Xi,Y,Xj), _, Out) :- !,
	(Xj = s(_) -> 
	    tab(Out, 4), write(Out, 'static /*Structure*/Term ')
	    ;
	    tab(Out, 8)
	),
	write_reg(Xj, Out),
	write(Out, ' = makeStructure('),
	write_reg(Xi, Out),
	write(Out, ', '),
	write_reg(Y, Out),
	write(Out, ');'), nl(Out).
...
write_java0(get_val(Xi,Xj), _, Out) :- !,
	tab(Out, 8),
	write(Out, 'if (! unify('), write_reg(Xi, Out), write(Out, ','),
	write_reg(Xj, Out), write(Out, '))'), nl(Out),
	tab(Out, 12),
	write(Out, 'return fail(engine);'), nl(Out).
...
write_java0(get_int(N,Xi,Xj), In, Out) :- !,
	...
	% read mode
	tab(Out, 8), 
	write(Out, 'if (isInteger('), write_reg(Xj, Out), write(Out, ')){'), nl(Out),
	tab(Out, 12), 
	write(Out, 'if (intValue( '), write_reg(Xj, Out), write(Out, ') != '),
	write(Out, N), write(Out, ')'), nl(Out),
	tab(Out, 16),
 	write(Out, 'return fail(engine);'), nl(Out),
	% write mode
	tab(Out, 8),
   write(Out, '} else if (isVariable('), write_reg(Xj, Out), write(Out, ')){'), nl(Out),
	tab(Out, 12),
	write(Out, 'bind(/*VAR*/ '), write_reg(Xj, Out), write(Out, ','),
	write_reg(Xi, Out), write(Out, ');'), nl(Out),
	tab(Out, 8),
	% otherwise fail
...
write_java0(get_float(N,Xi,Xj), In, Out) :- !,
	...
	% read mode
	tab(Out, 8), 
	write(Out, 'if (isDouble('), write_reg(Xj, Out), write(Out, ')){'), nl(Out),
	tab(Out, 12), 
	write(Out, 'if (doubleValue( '), write_reg(Xj, Out), write(Out, ') != '),
	write(Out, N), write(Out, ')'), nl(Out),
	tab(Out, 16),
 	write(Out, 'return fail(engine);'), nl(Out),
	% write mode
...
write_java0(get_con(_,Xi,Xj), In, Out) :- !,
	...
	% read mode
	tab(Out, 8), 
	write(Out, 'if (isAtomTerm('), write_reg(Xj, Out), write(Out, ')){'), nl(Out),
	tab(Out, 12), 
	write(Out, 'if (! prologEquals('), 
	write_reg(Xj, Out), write(Out, ','), write_reg(Xi, Out), 
	write(Out, '))'), nl(Out),
	tab(Out, 16),
 	write(Out, 'return fail(engine);'), nl(Out),
	% write mode
...
write_java0(get_list(X), In, Out) :- !,
	...
	% read mode
	tab(Out, 8), 
	write(Out, 'if (isListTerm('), write_reg(X, Out), write(Out, ')){'), nl(Out),
	tab(Out, 12), 
	write(Out, 'Term[] args = consArgs('), write_reg(X, Out), write(Out, ');'), nl(Out),

	write_unify_read(Us, 0, Out), %%% TODO optimize to not make array
	% write mode
	tab(Out, 8),
   write(Out, '} else if (isVariable('), write_reg(X, Out), write(Out, ')){'), nl(Out),
	write_unify_write(Us, Rs, Out),
	tab(Out, 12),
	write(Out, ' bind('), write_reg(X, Out), write(Out, ',makeList('),
	write_reg_args(Rs, Out), write(Out, '));'), nl(Out),
	% otherwise fail
	tab(Out, 8),
 	write(Out, '} else {'), nl(Out),
	tab(Out, 12),
...
write_java0(get_list(X), In, Out) :- !,
	...
	write_unify_read_cons(X ,Us, 0, Out), %%% TODO optimize to not make array
	% write mode
	tab(Out, 8),
   write(Out, '} else if (isVariable('), write_reg(X, Out), write(Out, ')){'), nl(Out),
	write_unify_write(Us, Rs, Out),
...
write_java0(get_str(_F/A,Xi,Xj), In, Out) :- !,
	...
	% read mode
	tab(Out, 8), 
	write(Out, 'if (isCompound('), write_reg(Xj, Out), write(Out, ')){'), nl(Out), %??? == F
	tab(Out, 12),
	write(Out, 'if (!functorOf( '), write_reg(Xi, Out),
	write(Out, ','), write_reg(Xj, Out),
	write(Out, '))'), nl(Out),
	tab(Out, 16),
	write(Out, 'return fail(engine);'), nl(Out),
	tab(Out, 12), 
	write(Out, 'Term[] args = args('),
	write_reg(Xj, Out), write(Out, ');'), nl(Out),
   ...
   write(Out, '} else if (isVariable('), write_reg(Xj, Out), write(Out, ')){'), nl(Out),
	...
	tab(Out, 12),
	write(Out, 'bind(/*VAR*/ '), write_reg(Xj, Out), write(Out, ',makeStructure('),
	write_reg(Xi, Out), write(Out, ', args));'), nl(Out),
	% otherwise fail
	tab(Out, 8),
 	write(Out, '} else {'), nl(Out),
	tab(Out, 12),
...
write_label(main(F/A, Modifier), Out) :- !,
	...
	write(Out, 'class '), 
	write_class_name(F/A, Out), 
	write(Out, ' extends PredicateBase {'), nl(Out).
write_label(F/A, Out) :- !,
	...
	nl(Out),
	tab(Out, 4),
	write(Out, 'public String nameUQ() { return "'),
	write_constant( F,Out),
	write(Out, '\"; }'), nl(Out),

   % setArg method
   nl(Out), 
   write_setarg(F/A, Out), nl(Out),

   % getArg method
   nl(Out), 
   write_getarg(F/A, Out), nl(Out),

    % toString method
	nl(Out),
	tab(Out, 4),
	write(Out, 'public String toPrologString(java.util.Collection newParam) {'), nl(Out),
	tab(Out, 8),
	write(Out, 'return "\''), write_constant(F, Out), 
	(A > 0 ->
	    write_enum('\'(" + argString(', arg, 1, A, ',newParam) + "," + argString(', ',newParam) + ")',0, Out)
	    ;
	    true
	),
	write(Out, '";'), nl(Out), 
	...
	nl(Out),
	tab(Out, 4),
	write(Out, 'public Predicate exec(Prolog engine) { enter(engine); Term[] engine_aregs = engine.getAreg(); '), nl(Out).
write_label(L, Out) :-
	tab(Out, 4),
	write(Out, '}'), nl(Out),
	write(Out, '}'), nl(Out),
	...
	write(Out, ' {'), nl(Out),
	tab(Out, 4),
	write(Out, 'public Predicate exec(Prolog engine) { enter(engine); Term[] engine_aregs = engine.getAreg(); //anony '), nl(Out), !.
...
%%% Read Mode
write_unify_read_cons(_CONS,[], _, _) :- !.
write_unify_read_cons(CONS,[unify_void(I)|Xs], N, Out) :- !,
	N1 is N+I,
	write_unify_read_cons(CONS,Xs, N1, Out).
write_unify_read_cons(CONS,[X|Xs], N, Out) :-
	write_unify_r_cons(CONS,X, N, Out),
	N1 is N+1,
	write_unify_read_cons(CONS,Xs, N1, Out).

write_unify_r_cons(_CONS,A,B,C):-write_unify_r(A,B,C).
...
write_inline0(fail, _, Out) :- !,
	tab(Out, 8), write(Out, 'return fail(engine);'), nl(Out).
write_inline0('$get_level'(X), _, Out) :- !,
	write_if_fail(op('!', unify(X,#('makeInteger'('engine.B0')))), [], 8, Out).
...
write_inline0('$cut'(X), _, Out)       :- !,
	...
	tab(Out, 8),
	write(Out, 'if (! isCutter/*Integer*/('), write_reg(X, Out), write(Out, ')) {'), nl(Out),
	tab(Out, 12),
	...
	write(Out, '} else {'), nl(Out),
	tab(Out, 12),
	write(Out, 'engine.cut(( '), write_reg(X, Out), 
	write(Out, '));'), nl(Out),
	tab(Out, 8),
	write(Out, '}'), nl(Out).
...
write_inline0(list(X), _, Out)     :- !, write_if_fail(op('!', @('isListTerm'(X))), [X], 8, Out).
write_inline0(compound(X), _, Out)     :- !, write_if_fail(op('!', @('isCompound'(X))), [X], 8, Out).

% Type testing
write_inline0(var(X), _, Out)     :- !, write_if_fail(op('!', @('isVariable'(X))), [X], 8, Out).
write_inline0(atom(X), _, Out)    :- !, write_if_fail(op('!', @('isAtomTerm'(X))), [X], 8, Out).
...
write_inline0(atomic(X), _, Out) :- !, 
	write_if_fail(op('&&',op('!',@('isAtomTerm'(X))), op('!',@('isNumber'(X)))), [X], 8, Out).

write_inline0(java(X,Y), _, Out) :- !, 
	write_if_fail(op('!', @('isJavaObject'(X))), [X], 8, Out),
%%   EXP = #('makeAtom'(@(getName(@(getClass(@(object(cast('Term',X))))))))),
	EXP = #('makeAtom'(@(getName(@(getClass(@(object(X)))))))),
	write_if_fail(op('!', unify(Y,EXP)), [], 8, Out).


write_inline0(ground(X), _, Out) :- !, write_if_fail(op('!', @('isGround'(X))), [X], 8, Out).
% Term comparison
write_inline0('$equality_of_term'(X,Y), _, Out)   :- !, write_if_fail(op('!',@('prologEquals'(X,Y))), [X,Y], 8, Out).
write_inline0('$inequality_of_term'(X,Y), _, Out) :- !, write_if_fail(@('prologEquals'(X,Y)), [X,Y], 8, Out).
write_inline0('$after'(X,Y), _, Out)              :- !, write_if_fail(op('<=',@('compareTerm'(X,Y)),0), [X,Y], 8, Out).
write_inline0('$before'(X,Y), _, Out)             :- !, write_if_fail(op('>=',@('compareTerm'(X,Y)),0), [X,Y], 8, Out).
write_inline0('$not_after'(X,Y), _, Out)          :- !, write_if_fail(op('>', @('compareTerm'(X,Y)),0), [X,Y], 8, Out).
write_inline0('$not_before'(X,Y), _, Out)         :- !, write_if_fail(op('<', @('compareTerm'(X,Y)),0), [X,Y], 8, Out).
write_inline0('$identical_or_cannot_unify'(X,Y), _, Out) :- !,
	write_if_fail(op('&&', op('!',@('prologEquals'(X,Y))), unify(X,Y)), [X,Y], 8, Out).
...
write_if_fail(Cond, Args, Tab, Out) :- nonvar(Cond), ground(Args), !,
	EXP  = if_then(Cond, 'return fail(engine)'),
...
write_arith(M, E, V, Tab, Out) :- 
	...
	SENT = if_then(op('!', unify(V,A)), 'return fail(engine)'),
...
write_arith_compare(M, E1, E2, Tab, Out) :- 
	...
	A = @(A0),
	EXP = try_catch(SENT, 'BuiltinException', ['e.goal = this','throw e']),
	SENT = if_then(op(M, A, 0), 'return fail(engine)'),
	%write_deref_args([E1,E2], Out),
	write_inline_java(EXP, Tab, Out).
...
write_reg(X, _) :- var(X), !,
	...
	fail.
write_reg(void,   Out) :- !, write(Out, 'engine.makeVariable(this)').
write_reg(ea(X),  Out) :- !, write(Out, 'engine_aregs['), write(Out, X), write(Out, ']').
...
/*****************************************************************
  Write setarg
*****************************************************************/
write_setarg(_F/A, Out) :- 
	tab(Out, 4), write(Out, 'public void sArg(int i0, Term val) {  switch (i0) {'), 
	A > 0,
	for(I, 1, A),
            succ(I0,I),
	    tab(Out, 8),
           write(Out, ' case '),write(Out, I0), write(Out, ': '),             
	    write(Out, arg), write(Out, I), write(Out, ' = '), write(Out, 'val;'),
	    write(Out, 'break ;'),  nl(Out),
	fail.

write_setarg(_F/_A, Out) :- !,
            write(Out, 'default: newIndexOutOfBoundsException("setarg" , i0 , val);  }   }'),
            nl(Out).

/*****************************************************************
  Write getarg
*****************************************************************/
write_getarg(_F/A, Out) :- 
	tab(Out, 4), write(Out, 'public Term gArg(int i0) {  switch (i0) {'), 
	A > 0,
	for(I, 1, A),
            succ(I0,I),
	    tab(Out, 8),
           write(Out, ' case '),write(Out, I0), write(Out, ': return '),             
	    write(Out, arg), write(Out, I),write(Out, ';'),
	    nl(Out),
	fail.

write_getarg(_F/_A, Out) :- !,
            write(Out, 'default: return newIndexOutOfBoundsException("getarg", i0,null);  }   }'),
            nl(Out).


do_debug:-!.
do_debug:-guitracer,trace.
