  ...
  */
  ...
  public Predicate exec(Prolog engine) {
    ...
    */
    ...
    if (!isAtomTerm(a1)) throw new IllegalDomainException(this, 1, "source_sink", a1);
    ...
    try {
      if (prologEquals(a2, PRED_open_4.SYM_READ)) {
        if (!file.exists()) throw new ExistenceException(this, 1, "source_sink", a1, "");
    ...
    if (engine.getStreamManager().containsKey(streamObject)) throw new InternalException("stream object is duplicated");
    a4 = deref(a4);
    for (Object tmp = a4; !isNil(tmp); tmp = deref(rest((Object) tmp))) {
      if (isVariable(tmp)) throw new PInstantiationException(this, 4);
      if (!isListTerm(tmp)) throw new IllegalTypeException(this, 4, "list", a4);
      final Object car = deref(first((Object) tmp));
      if (isVariable(car)) throw new PInstantiationException(this, 4);
      if (isCompound(car)) {
        ...
        if (prologEquals(functor, PRED_open_4.SYM_ALIAS_1)) {
          alias = deref(args[0]);
          if (!isAtomTerm(alias)) throw new IllegalDomainException(this, 4, "stream_option", car);
          if (!engine.getStreamManager().containsKey(alias)) continue;
