  ...
  public Predicate exec(Prolog engine) {
    ...
    */
    ...
    if (prologEquals(a1, PRED_sort_2.Nil)) {
      if (!unify(a2, PRED_sort_2.Nil)) return this.fail(engine);
      return this.cont;
    }
    ...
    if (!prologEquals(tmp, PRED_sort_2.Nil)) throw new PInstantiationException(this, 1);
    try {
    ...
    for (int i = list.length - 1; i >= 0; i--) {
      if (!prologEquals(list[i], tmp2)) tmp = makeList(list[i], tmp);
      tmp2 = list[i];
    }
