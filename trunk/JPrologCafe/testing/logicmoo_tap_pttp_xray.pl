...
ontology_hlPredicateAttribute(logicalConnective,pttp_skolem).
...
trace_search :-                         % enables pttp_search progress reports
        retract(trace_search_progress_pred(_)),
        fail.
trace_search :-
        assert(trace_search_progress_pred(write_search_progress)).

dont_trace_search :-                    % disables pttp_search progress reports
        retract(trace_search_progress_pred(_)),
        fail.
dont_trace_search :-
...
pttp_request(M) :-                             % call pttp_request with depth bound M
	(compile_complete_search, compile_explaination_printing , lemma_handling_flag) ->
	        pttp_request(M,_N,_LemExplaination,_LemExplainationEnd,_Explaination,_ExplainationEnd);
        (compile_complete_search, (compile_explaination_printing ; lemma_handling_flag)) -> 
                pttp_request(M,_N,_Explaination,_ExplainationEnd);
        compile_complete_search ->
                pttp_request(M,_N).

pttp_request :-                                % unbounded pttp_search of pttp_request
        (compile_complete_search ->
	    pttp_request(1000000);
	%true ->
	    ((compile_explaination_printing , lemma_handling_flag) ->
	         pttp_request(_LemExplaination,_LemExplainationEnd,_Explaination,_ExplainationEnd);
	     (compile_explaination_printing ; lemma_handling_flag) -> 
		 pttp_request(Explaination,_ExplainationEnd);
             %true ->
		 pttp_request)).
...
/*
get_relations(Context,Ctx):-
	mooCache(R, _, Axiom, Vars, Context, Ctx, Tracking, Agent, Status),
	get_relation(R),
...
getArity(R,A):-
	mooCache(valence, surface, valence(R,A), Vars, ContextName, Context, Tracking, Agent, Status).
getArity(R,A):-
	mooCache(R, surface, P, Vars, ContextName, Context, Tracking, Agent, Status),
	functor(P,R,A).
getArity(R,A):-
	mooCache(R, GAF, Vars, ContextName, Context, Tracking, Agent),
...
dpttp3(Name) :-
	...
	writeSTDERR('done.'),
        ...
        writeSTDERR('XRay compiling pttp_request ... '),
...
%%% complete pttp_search facilities are turned (during compile-time)
...
pttp_configuration :-
	...
	(count_beliefs_pred(true) ->
	    ...
	    format('PTTP does not true pttp_search progress.');
	    format('PTTP traces pttp_search progress!')),
	(compile_explaination_printing ->
	    format('PTTP compiles explaination printing!');
	    format('PTTP does not compile explaination printing.')),
	(compile_complete_search ->
	    format('PTTP compiles complete pttp_search!');
	    format('PTTP does not compile complete pttp_search.')).
...
hook_configuration :-
	writeSTDERR('HOOK CONFIGURATION:'),
	writeSTDERR('body hook handling'),
	isMooOption(Agent,body_hooks_flag,R),
	writeSTDERR(isMooOption(Agent,body_hooks_flag,R)),
	writeSTDERR('predicate hook handling'),
	isMooOption(Agent,pred_hooks_flag,R),writeSTDERR(isMooOption(Agent,pred_hooks_flag,R)).

%%% Hook handling is turned off by no_hook_handling

no_hook_handling :- setMooOption(Agent,body_hooks_flag,false), setMooOption(Agent,pred_hooks_flag,false).

%%% Body hooks are turned on by setMooOption(Agent,body_hooks_flag,true).
%%% off by setMooOption(Agent,body_hooks_flag,false).

%%% Predicate hooks are turned on by setMooOption(Agent,pred_hooks_flag,true).
%%% off by setMooOption(Agent,pred_hooks_flag,false).
...
write_search_progress(Level) :-
        ...
        writeSTDERR(Level),
        writeSTDERR(' pttp_search...  ').

%%% A pttp_request can be timed by time(pttp_request).
...
add_body_hooks((Head :- Body),(Head :- Body2)) :-
	isMooOption(Agent,body_hooks_flag,true),
	!,
	(bhook1_p(Head :- Body) ->
...
phook_tests(P,N,TestsA,Proc,ProcP) :-
	isMooOption(Agent,pred_hooks_flag,true),
	!,
...
pttp_ask(Name,Request) :-
	...
	dpttp1((pttp_request:-Request),Q,Matrix,Request1:Matrix),

	
        writeSTDERR('XRay writing pttp_request ... '),
...
add_lemmatization_p(Head :- Body) :-
	lemma_flag,
	!,
	(functor(Head,pttp_request,_) -> fail;
...
lemma_tests(P,N,Result) :-
	lemma_handling_flag,
	lemma_tests_p(P,N),     % for avoiding problems with pttp_request/0
	!,
...
%% 28/06/96 updated pttp_cnf,make_matrix, make_clause
%%                                                                           %%
%% Purpose:                                                                  %%
%%                                                                           %%
%% Maintainer:  Torsten Schaub                                                   %%
...
subs_reduction([C1|M1],M) :-
	...
	(pttp_subsumes(C1,M2,M3) ->
	    M = [C1|M3];
	%true ->
	    M = M2).

pttp_subsumes(_,[],[]).
pttp_subsumes(C1,[C2|_],_) :-
	subset(C2,C1),
	!,
	fail.
pttp_subsumes(C,[C1|M1],M) :-
	pttp_subsumes(C,M1,M2),
	!,
	(subset(C,C1) ->
	        M=M2;
	%true ->
...
%%% the body of the transformed clause, using the sound `pttp_unify'
%%% predicate.
%%%
%%% For example,
%%%    p(X,Y,f(X,Y)) :- true.
%%% is transformed into
%%%    p(X,Y,f(X1,Y1)) :- pttp_unify(X,X1), pttp_unify(Y,Y1).
...
linearize(TermIn,TermOut,VarsIn,VarsOut,MatchesIn,MatchesOut) :-
        ...
        identical_member(TermIn,VarsIn) ->
                VarsOut = VarsIn,
                conjoin(MatchesIn,pttp_unify(TermIn,TermOut),MatchesOut);
        %true ->
                TermOut = TermIn,
...
pttp_unify(X,Y) :- unify_with_occurs_check(X,Y).


%%% Depth-first iterative-deepening pttp_search.
%%%
%%% `add_complete_search' adds arguments DepthIn and DepthOut
%%% to each PTTP literal to control bounded depth-first
%%% pttp_search.  When a literal is called,
...
add_complete_search_args(Body,DepthIn,DepthOut,Body1) :-
        ...
        Body = (A ; B) ->
                ...
                %true ->
                        ...
                        add_complete_search_args(B,DepthIn,DepthOut,B1)),
                disjoin(A1,B1,Body1);
        Body = pttp_search(Goal,Max,Min,Inc) ->
                PrevInc is Min + 1,
                add_complete_search_args(Goal,DepthIn1,DepthOut1,Goal1),
                DepthIn = DepthOut,
                Body1 = pttp_search(Goal1,Max,Min,Inc,PrevInc,DepthIn1,DepthOut1);
        Body = pttp_search(Goal,Max,Min) ->
                add_complete_search_args(pttp_search(Goal,Max,Min,1),DepthIn,DepthOut,Body1);
        Body = pttp_search(Goal,Max) ->
                add_complete_search_args(pttp_search(Goal,Max,0),DepthIn,DepthOut,Body1);
        Body = pttp_search(Goal) ->
                add_complete_search_args(pttp_search(Goal,1000000),DepthIn,DepthOut,Body1);
...
%%%
%%% Propositional goals are not counted into the pttp_search cost so
%%% that fully propositional problems can be solved without
%%% deepening when iterative-deepening pttp_search is used.
...
search_cost(Body,N) :-
        ...
        Body = (A , B) ->
                (A = search_cost(M) ->  % if first conjunct is search_cost(M),
                        N = M;          % pttp_search cost of the entire conjunction is M
                %true ->
                        search_cost(A,N1),
                        search_cost(B,N2),
                        N is N1 + N2);
...
%%% Depth-first iterative-deepening pttp_search can be
%%% specified for a goal by wrapping it in a call
%%% on the pttp_search predicate:
%%%    pttp_search(Goal,Max,Min,Inc)
%%% Max is the maximum depth to pttp_search (defaults to a big number),
%%% Min is the minimum depth to pttp_search (defaults to 0),
%%% Inc is the amount to increment the bound each time (defaults to 1).
%%%
%%% Depth-first iterative deepening pttp_search can be
%%% specified inside the PTTP formula by compiling
%%%   pttp_request :- pttp_search(p(b,a,c),Max,Min,Inc)
%%% and executing
%%%   pttp_request.
%%% or directly by the user by compiling
%%%   pttp_request :- p(b,a,c))
%%% and executing
%%%   pttp_search(pttp_request,Max,Min,Inc).
%%%
%%% The pttp_search(Goal,Max,Min,Inc) predicate adds
...
pttp_search(Goal,Max,Min,Inc) :-
        ...
        %true ->
                Goal2 = Goal1),
        !,
        pttp_search(Goal2,Max,Min,Inc,PrevInc,DepthIn,DepthOut).

pttp_search(Goal,Max,Min) :-
        pttp_search(Goal,Max,Min,1).

pttp_search(Goal,Max) :-
        pttp_search(Goal,Max,0).

pttp_search(Goal) :-
        pttp_search(Goal,1000000).

%%% Actual pttp_search driver predicate.
%%% Note that depth-bounded execution of Goal is enabled by
%%% the fact that the DepthIn and DepthOut arguments of
%%% pttp_search are also the DepthIn and DepthOut arguments of Goal.

pttp_search(_Goal,Max,Min,_Inc,_PrevInc,_DepthIn,_DepthOut) :-
        Min > Max,
        !,
        fail.
pttp_search(Goal,_Max,Min,_Inc,PrevInc,DepthIn,DepthOut) :-
        trace_search_progress_pred(P1),
        L1 =.. [P1,Min],
        call(L1),
        DepthIn = Min,
        call(Goal),
        DepthOut < PrevInc.   % fail if this solution was found in previous pttp_search
pttp_search(Goal,Max,Min,Inc,_PrevInc,DepthIn,DepthOut) :-
        Min1 is Min + Inc,
        pttp_search(Goal,Max,Min1,Inc,Inc,DepthIn,DepthOut).
...
predicates(Wff,L) :-
        ...
        Wff = (A ; B) ->
                ...
                predicates(B,L2),
                union(L2,L1,L);
        functor(Wff,pttp_search,_) ->        % list predicates in first argument of pttp_search
                arg(1,Wff,X),
                predicates(X,L);
        pttp_builtin(Wff) ->
                L = [];
...
add_answer_preds((pttp_request :- Request),(pttp_request :- (Request,true,true,writeSTDERR(answer:Vars)))) :-
	!,
...
constants(Wff,L) :-
        ...
        Wff = (A : B) ->
                ...
                constants(B,L2),
                union(L2,L1,L);
        functor(Wff,pttp_search,_) ->        % list constants in first argument of pttp_search
                arg(1,Wff,X),
                constants(X,L);
        pttp_builtin(Wff) ->
                L = [];
...
variables(Wff,L) :-
        ...
        Wff = (A : B) ->
                ...
                variables(B,L2),
                union(L2,L1,L);
        functor(Wff,pttp_search,_) ->        % list variables in first argument of pttp_search
                arg(1,Wff,X),
                variables(X,L);
        pttp_builtin(Wff) ->
                L = [];
...
pttp_skolem([],Term,Term).
pttp_skolem([Var|Vars],Term,SkTerm) :-
	pttp_skolem(Vars,Term,Term1),
	SkTerm=(Var^Term1).

instances(Clause,Terms,Instances) :-
	variablize_clause(Clause,Vars),
	pttp_skolem(Vars,instance(Vars,Terms,Clause),DoIt),
...
classical_clauses(WffI,WffO) :-
        ...
        WffI = (A ; B) ->
                ...
                classical_clauses(B,B1),
		disjoin(A1,B1,WffO);
        WffI = (A :- B) ->                        % ??? (special case pttp_request elim. TS Apr04)
	        WffO = true;
        pttp_builtin(WffI) ->
                WffO = true;
        %true ->
...
:- do_compile_complete_search.            % default is to compile complete pttp_search
:- admissibility_first.                     % default is to check admissibility first
:- verbose_mode.                         % default is to print explaination
:- setMooOption(Agent,body_hooks_flag,true).
:- setMooOption(Agent,body_hooks_flag,false).                      % default is no body hooks
:- setMooOption(Agent,pred_hooks_flag,true).
:- setMooOption(Agent,pred_hooks_flag,false).                      % default is no predicate hooks
...
pttp_builtin(writeSTDERR,1).
pttp_builtin(true,0).
pttp_builtin(deduce_by,_).
pttp_builtin(write_proved,_).
pttp_builtin(pttp_search,_).
pttp_builtin(search_cost,_).
pttp_builtin(pttp_unify,_).
pttp_builtin(identical_member,_).
pttp_builtin(unifiable_member,_).
pttp_builtin(inc_ncalls,0).
% --- compatibility predicates
pttp_builtin(justification,_).
pttp_builtin(compatible,_).
pttp_builtin(model_initialization,_).
% --- variable handling predicates
pttp_builtin(herbrand,1).
% hooks handling predicates
pttp_builtin(hook1,1).
pttp_builtin(hook2,1).
% --- lemma handling predicates
pttp_builtin(lemmatize,_).
pttp_builtin(dynamic_lemma,_).
pttp_builtin(static_lemma,_).
% --- misc
pttp_builtin(\+,1).
