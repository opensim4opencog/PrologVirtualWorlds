...
%createProcessedGoal(Goal):-servantProcessCreate((prolog_thread_at_exit(((thread_self(Id),prolog_thread_exit(i_am_done(Id))))),Goal),Id,[]).
...
%debugProcess(T):- prolog_thread_signal(T, (attach_console,trace, true)).
