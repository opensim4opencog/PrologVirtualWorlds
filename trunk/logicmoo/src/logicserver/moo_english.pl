...
parse :-
  ...
  asserta(parsing),
  writeDebug('compiler will produce code for parsing only').

generate :-
  retractall(parsing),
  asserta(generating),
  writeDebug('compiler will produce code for generation only').
  
parse_and_gen :-
  asserta(parsing),
  asserta(generating),
  writeDebug('compiler will produce code for parsing and generation').

lex_consult :-
  writeDebug('In SWI Prolog, ALE always consults lexicon').

lex_compile :-
  writeDebug('Not available in ALE for SWI Prolog').
  
interp :-
  retractall(no_interpreter),
  writeDebug('interpreter is active').

nointerp :-
  asserta(no_interpreter),
  writeDebug('interpreter is inactive').

subtest :-
  retractall(no_subsumption),
  compile_subsume,
  writeDebug('edge/empty category subsumption checking active').

nosubtest :-
  asserta(no_subsumption),
  writeDebug('edge/empty category subsumption checking inactive').
...
noadderrs :-
  ...
  writeDebug('Errors from adding descriptions will be suppressed.').

adderrs :-
  retractall(suppress_adderrs),
  writeDebug('Errors from adding descriptions will be displayed.').
...
(unify_type(a_ X,a_ X,a_ X) if_h).  % subsumes_chk/2 unhooks the getPrologVars - need
                                    % to do it ourselves
...
add_to(Ref1-SVs1,Ref2,SVs2,IqsIn,IqsOut):-
  ...
  if((deref(Ref1,SVs1,Ref3,SVs3),
     ...
     ; error_msg((writeDebug('add_to could not unify '),
             \+ \+ (duplicates_list([Ref1-SVs1,Ref2-SVs2],IqsIn,[],_,[],_,0,_),
                    tab(5),pp_fs(Ref1-SVs1,[],VisMid,5), 
                     writeDebug('and '), pp_fs(Ref2-SVs2,VisMid,VisOut,5),
...
add_to(Path1 == Path2,Tag,SVs,IqsIn,IqsOut):-
  ...
  if(u(SVsAtPath1,SVsAtPath2,TagAtPath1,TagAtPath2,IqsMid2,IqsOut),
     true,
     (suppress_adderrs -> fail
     ; error_msg((writeDebug('add_to could not unify paths '), 
                 writeDebug(Path1), writeDebug(' and '), 
                 writeDebug(Path2), writeDebug(' in '),
                 pp_fs(Tag-SVs,IqsIn,5),
                 ttynl)))).
    ...
    -> IqsOut = IqsMid
     ; ((IqOut \== done)
        -> IqsOut = [IqOut|IqsMid]
         ; (suppress_adderrs 
            -> fail
             ; error_msg((writeDebug('add_to could not inequate '),
                   pp_fs(Tag2-bot,[],5),
                   writeDebug('and '),
                   pp_fs(Tag-SVs,IqsIn,5),
                   ttynl))))).
...
add_to(Feat:Desc,Ref,SVs,IqsIn,IqsOut):-
  ...
  if(featval(Feat,SVs,Ref,FSatFeat,IqsIn,IqsMid), 
     ...
     (suppress_adderrs
      -> fail
       ; error_msg((writeDebug('add_to could not add feature '), writeDebug(Feat), 
                    writeDebug(' to '), pp_fs(Ref-SVs,IqsIn,5),
                    ttynl)))).
...
add_to(@ MacroName,Ref,SVs,IqsIn,IqsOut):-
  ...
  if((MacroName macro Desc),
     ...
     error_msg((writeDebug('add_to could not add undefined macro '),
                writeDebug(MacroName),
                writeDebug(' to '), pp_fs(Ref-SVs,IqsIn,5),
                ttynl))).
add_to(Type,Ref,SVs,IqsIn,IqsOut):-
  ...
  if(add_to_type(Type,SVs,Ref,IqsIn,IqsOut),
     ...
     (suppress_adderrs
      -> fail
       ; error_msg((writeDebug('add_to could not add incompatible type '), 
                   writeDebug(Type),
                   writeDebug('to '), pp_fs(Ref-SVs,IqsIn,5),
                   ttynl)))).
...
add_to(Atom,Ref,SVs,Iqs,Iqs) :-
  atomic(Atom),
  !, 
  error_msg((writeDebug('add_to could not add undefined type '), writeDebug(Atom),
             writeDebug('to '), pp_fs(Ref-SVs,Iqs,5),
             ttynl)).
add_to(Desc,Ref,SVs,Iqs,Iqs):-
  error_msg((writeDebug('add_to could not add ill formed complex description '), 
            tab(5), writeDebug(Desc),
            writeDebug('to '),
            pp_fs(Ref-SVs,Iqs,5),
            ttynl)).
...
pathval([Feat|Path],Tag,SVs,TagOut,SVsOut,IqsIn,IqsOut):-
  if(featval(Feat,SVs,Tag,FSMid,IqsIn,IqsMid),
     ...
     (suppress_adderrs
      -> fail
       ; (writeDebug('feature '), writeDebug(Feat), writeDebug(' in path '), 
          writeDebug([Feat|Path]), writeDebug('could not be added to '), 
          pp_fs(Tag-SVs,[],5),
          fail))).
...
match_list(Sort,_,_,_,_,_,_,_,_,_,_,_) :-
  error_msg((writeDebug('error: cats> value with sort, ':Sort),
            writeDebug(' is not a valid list argument'))).
...
print_edge_discard(LReln,I,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
 pp_fs(Tag-SVs,Iqs),
  writeDebug('Edge created for category above:'),
%  writeDebug('     index: ':I),
  writeDebug('      from: ':Left:' to: ':Right),
  writeDebug('    string: '),write_out(Left,Right),
  writeDebug('      rule: ':RuleName),
  writeDebug(' # of dtrs: ':ND),
  print_edge_discard_act(LReln,I),
 request_discard(LReln,I,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND).

print_edge_discard_act(<,I) :-
  !,writeDebug('is equal to an existing edge, index:':I:'.').
print_edge_discard_act(#,I) :-
  writeDebug('is subsumed by an existing edge, index:':I:'.').

request_discard(_,_,_,_,_,_,_,_,_,_) :-
  go(_),
  !.
request_discard(LReln,I,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
  writeDebug('Action(noadd,continue,break,dtr-#,existing,abort)? '),
 read(Response),
  request_discard_act(Response,LReln,I,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND).

request_discard_act(noadd,_,_,_,_,_,_,_,_,_,_) :- !.
request_discard_act(continue,_,_,_,_,_,_,_,_,_,_) :- 
  !,fail.
request_discard_act(break,LReln,I,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
  !,break,
  print_edge_discard(LReln,I,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND).
request_discard_act(dtr-D,LReln,I,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
  ...
  print_edge_discard(LReln,I,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND).
request_discard_act(existing,LReln,I,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
  clause(edge(I,Left,Right,ETag,ESVs,EIqs,EDtrs,ERuleName),true),
  !,edge_act(I,Left,Right,ETag,ESVs,EIqs,EDtrs,ERuleName),  
  print_edge_discard(LReln,I,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND).
request_discard_act(abort,_,_,_,_,_,_,_,_,_,_) :-
  !,abort.
request_discard_act(_,LReln,I,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
  request_discard(LReln,I,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND).
...
print_edge_retract(I,Left,Right,ETag,ESVs,EIqs,EDtrs,ERuleName,NED,
                   Tag,SVs,Iqs,Dtrs,RuleName) :-
 pp_fs(ETag-ESVs,EIqs),
  writeDebug('Edge created for category above:'),
  writeDebug('     index: ':I),
  writeDebug('      from: ':Left:' to: ':Right),
  writeDebug('    string: '),write_out(Left,Right),
  writeDebug('      rule: ':ERuleName),
  writeDebug(' # of dtrs: ':NED),
  writeDebug('is subsumed by an incoming edge.'),
 request_retract(Left,I,Right,ETag,ESVs,EIqs,EDtrs,ERuleName,NED,
                   Tag,SVs,Iqs,Dtrs,RuleName).

request_retract(Left,I,_,_,_,_,_,_,_,_,_,_,_,_) :-
  go(_),
  retract(edge(I,Left,_,_,_,_,_,_)),
  retract(to_rebuild(I)),
  !,fail.
request_retract(Left,I,Right,ETag,ESVs,EIqs,EDtrs,ERuleName,NED,
              Tag,SVs,Iqs,Dtrs,RuleName) :-
  writeDebug('Action(retract,continue,break,dtr-#,incoming,abort)? '),
 read(Response),
  request_retract_act(Response,Left,I,Right,ETag,ESVs,EIqs,EDtrs,ERuleName,NED,
                    Tag,SVs,Iqs,Dtrs,RuleName).
request_retract_act(retract,Left,I,_,_,_,_,_,_,_,_,_,_,_,_) :-
  retract(edge(I,Left,_,_,_,_,_,_)),
  retract(to_rebuild(I)),
  !,fail.
request_retract_act(remain,_,_,_,_,_,_,_,_,_,_,_,_,_,_) :-
  !,fail.
request_retract_act(continue,_,_,_,_,_,_,_,_,_,_,_,_,_,_) :-
  !.
request_retract_act(break,Left,I,Right,ETag,ESVs,EIqs,EDtrs,ERuleName,NED,
                  Tag,SVs,Iqs,Dtrs,RuleName) :-
  !,break,
  print_edge_retract(I,Left,Right,ETag,ESVs,EIqs,EDtrs,ERuleName,NED,
                     Tag,SVs,Iqs,Dtrs,RuleName).
request_retract_act(dtr-D,Left,I,Right,ETag,ESVs,EIqs,EDtrs,ERuleName,NED,
                  Tag,SVs,Iqs,Dtrs,RuleName) :-
  ...
  print_edge_retract(I,Left,Right,ETag,ESVs,EIqs,EDtrs,ERuleName,NED,
                     Tag,SVs,Iqs,Dtrs,RuleName).
request_retract_act(incoming,Left,I,Right,ETag,ESVs,EIqs,EDtrs,ERuleName,NED,
                  Tag,SVs,Iqs,Dtrs,RuleName) :-
  !,length(Dtrs,ND),
  (print_incoming_edge(Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND)
   -> true
    ; print_edge_retract(I,Left,Right,ETag,ESVs,EIqs,EDtrs,ERuleName,NED,
                         Tag,SVs,Iqs,Dtrs,RuleName)).
request_retract_act(abort,_,_,_,_,_,_,_,_,_,_,_,_,_,_) :-
  !,abort.
request_retract_act(_,Left,I,Right,ETag,ESVs,EIqs,EDtrs,ERuleName,NED,
                  Tag,SVs,Iqs,Dtrs,RuleName) :-
  request_retract(Left,I,Right,ETag,ESVs,EIqs,EDtrs,ERuleName,NED,
                Tag,SVs,Iqs,Dtrs,RuleName).
...
print_incoming_edge(Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
 pp_fs(Tag-SVs,Iqs),
  writeDebug('Incoming Edge: '),
  writeDebug('      from: ':Left:' to: ':Right),
  writeDebug('    string: '),write_out(Left,Right),
  writeDebug('      rule:  ':RuleName),
  writeDebug(' # of dtrs: ':ND),
  request_incoming_edge(Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND).

request_incoming_edge(Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
  writeDebug('Action(noadd,dtr-#,existing,abort)?' ),
 read(Response),
  request_incoming_act(Response,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND).
request_incoming_act(noadd,_,_,_,_,_,_,_,_) :-
  !.
request_incoming_act(existing,_,_,_,_,_,_,_,_) :-
  !,fail.
request_incoming_act(abort,_,_,_,_,_,_,_,_) :-
  !,abort.
request_incoming_act(dtr-D,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
  nth_index(Dtrs,D,DI,DLeft,DRight,DTag,DSVs,DIqs,DDtrs,DRule),
  !,length(DDtrs,DND),
  print_dtr_edge(D,DI,DLeft,DRight,DTag,DSVs,DIqs,DDtrs,DRule,DND),
  print_incoming_edge(Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND).
request_incoming_act(_,Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
  request_incoming_edge(Left,Right,Tag,SVs,Iqs,Dtrs,RuleName,ND).
...
print_edge(Left,Right,TagOut,SVsOut,IqsOut,Dtrs,RuleName,ND) :-
 pp_fs(TagOut-SVsOut,IqsOut),
  writeDebug('Edge created for category above: '),
  writeDebug('      from: ':Left:' to: ':Right),
  writeDebug('    string: '),write_out(Left,Right),
  writeDebug('      rule:  ':RuleName),
  writeDebug(' # of dtrs: ':ND),
 request_edge(Left,Right,TagOut,SVsOut,IqsOut,Dtrs,RuleName,ND).

request_edge(Left,Right,TagOut,SVsOut,IqsOut,Dtrs,RuleName,ND) :-
  go(Left),               % right-to-left parser triggers on left
  !,retractall(go(_)),
  request_edge(Left,Right,TagOut,SVsOut,IqsOut,Dtrs,RuleName,ND).
request_edge(_,_,_,_,_,_,_,_) :-
  go(_),
  !.
request_edge(Left,Right,TagOut,SVsOut,IqsOut,Dtrs,RuleName,ND) :-
  writeDebug('Action(add,noadd,go(-#),break,dtr-#,abort)? '),
 read(Response),
  request_edge_act(Response,Left,Right,TagOut,SVsOut,IqsOut,Dtrs,RuleName,ND).

request_edge_act(add,_,_,_,_,_,_,_,_) :-
  !.
request_edge_act(noadd,_,_,_,_,_,_,_,_) :- 
  !,fail.
request_edge_act(go,_,_,_,_,_,_,_,_) :-
  !,asserta(go(go)).
request_edge_act(go-G,_,_,_,_,_,_,_,_) :-
  !,asserta(go(G)).
request_edge_act(break,Left,Right,TagOut,SVsOut,IqsOut,Dtrs,RuleName,ND) :-
  !,break,
  print_edge(Left,Right,TagOut,SVsOut,IqsOut,Dtrs,RuleName,ND).
request_edge_act(dtr-D,Left,Right,TagOut,SVsOut,IqsOut,Dtrs,RuleName,ND):-
  nth_index(Dtrs,D,DI,DLeft,DRight,DTag,DSVs,DIqs,DDtrs,DRule),
  !,length(DDtrs,DND),
  print_dtr_edge(D,DI,DLeft,DRight,DTag,DSVs,DIqs,DDtrs,DRule,DND),
  print_edge(Left,Right,TagOut,SVsOut,IqsOut,Dtrs,RuleName,ND).
request_edge_act(abort,_,_,_,_,_,_,_,_) :-
  !,abort.
request_edge_act(_,Left,Right,TagOut,SVsOut,IqsOut,Dtrs,RuleName,ND) :-
  request_edge(Left,Right,TagOut,SVsOut,IqsOut,Dtrs,RuleName,ND).

print_dtr_edge(D,DI,DLeft,DRight,DTag,DSVs,DIqs,DDtrs,DRule,DND) :-
 pp_fs(DTag-DSVs,DIqs),
  writeDebug('Daughter number '), writeDebug(D),
  writeDebug('      from: ':DLeft:' to: ':DRight),
  writeDebug('    string: '),write_out(DLeft,DRight),
  writeDebug('      rule:  ':DRule),
  writeDebug(' # of dtrs: ':DND),
  request_dtr_edge(D,DI,DLeft,DRight,DTag,DSVs,DIqs,DDtrs,DRule,DND).

request_dtr_edge(D,I,DLeft,DRight,DTag,DSVs,DIqs,DDtrs,DRule,DND) :-
  writeDebug('Action(retract,dtr-#,parent,abort)?' ),
 read(Response),
  request_dtr_act(Response,D,I,DLeft,DRight,DTag,DSVs,DIqs,DDtrs,DRule,DND).
request_dtr_act(parent,_,_,_,_,_,_,_,_,_,_) :-
  !.
request_dtr_act(retract,_,I,DLeft,_,_,_,_,_,_,_) :-
  retract(edge(I,DLeft,_,_,_,_,_,_)),  % will fail on empty cats
  !.
request_dtr_act(abort,_,_,_,_,_,_,_,_,_,_) :-
  !,abort.
request_dtr_act(dtr-DD,D,I,Left,Right,Tag,SVs,Iqs,Dtrs,Rule,ND) :-
...
request_dtr_act(_,D,I,Left,Right,Tag,SVs,Iqs,Dtrs,Rule,ND) :-
  request_dtr_edge(D,I,Left,Right,Tag,SVs,Iqs,Dtrs,Rule,ND).
  ...
  -> DI = I,
     ...
     -> empty_cat(E,DLeft,DTag,DSVs,DIqs,DDtrs,DRule),
      ...
      ; (clause(edge(I,DLeft,DRight,DTag,DSVs,DIqs,DDtrs,DRule),true)
        -> true
         ; error_msg((writeDebug('edge has been retracted')))
        )
     )
   ; NMinus1 is N-1,
...
compile_body((\+ GD),Iqs,Iqs,[(\+ PGoal)|PGoalsRest],PGoalsRest,CBSafe,
             VarsIn,VarsIn,FSPal,FSs,FSs) :-  % getPrologVars will be unbound, so dont thread
...
lex_act(Word,Tag,SVs,IqsOut,WordStart,Desc) :-
  if(add_to(Desc,TagStart,bot,[],IqsMid),
     ...
     error_msg((writeDebug('lex: unsatisfiable lexical entry for '),
                writeDebug(WordStart)))).
...
me_subsume_act(<,_,A,_,AsRest,BsRest,NewAccum,[A|AsRest],MEs) :-
  writeDebug('EFD-closure discarded a subsumed empty category'),
  minimise_emptys(BsRest,NewAccum,MEs).
...
%     writeDebug('EFD-closure generated a partial rule'),
     split_emptys_rules(Items,NewRulesRest,NewRulesMid,NewEmptys)
   ; % Functor == empty,
     NewEmptys = [Item|NewEmptysMid],
%     writeDebug('EFD-closure generated an empty category'),
  ...
  -> arg(1,DSVs,HdFS),
     ...
     -> PrevDtrsMid = [empty(N,Node)|PrevDtrsRest],
      ...
      ; (sub_type(e_list,TlType)
        -> PrevDtrsMid = [empty(N,Node)|PrevDtrsMid2],
         ...
         ; error_msg((writeDebug('error: cats> value with sort, ':TlType),
                      writeDebug(' is not a valid list argument')))
        )
     )
   ; (sub_type(e_list,DtrsType)
     -> match_cat_to_next_cat(Rest,Mother,RuleName,PrevDtrs,PrevDtrsMid,
                              RuleIqs,Empty,EmptyorRule,Node)
      ; error_msg((writeDebug('error: cats> value with sort, ':DtrsType),
                   writeDebug(' is not a valid list argument')))
     )
  ).
match_cat_to_next_cat((cats> Dtrs),Mother,RuleName,PrevDtrs,PrevDtrsMid,
  ...
  -> arg(1,DSVs,HdFS),
   ...
   ; (sub_type(e_list,DtrsType)
     -> error_msg((writeDebug('error: rule ':RuleName),
                   writeDebug(' has no daughters')))
      ; error_msg((writeDebug('error: cats> value with sort, ':DtrsType),
                   writeDebug(' is not a valid list argument')))
     )
  ).
...
match_cat_to_next_cat((goal> GoalDesc,Rest),Mother,RuleName,PrevDtrs,
  ...
  request_goal(GoalDesc,Goal,RuleIqs,IqsMid),
  call(Goal),
  check_inequal(IqsMid,IqsMid2),
...
match_to_next_cat((goal> GoalDesc,Rest),Mother,RuleName,PrevDtrs,PrevDtrsMid,
  ...
  request_goal(GoalDesc,Goal,IqsIn,IqsMid),
  call(Goal),
  check_inequal(IqsMid,IqsMid2),
  match_to_next_cat(Rest,Mother,RuleName,PrevDtrs,PrevDtrsMid,IqsMid2,
                    EmptyorRule,Node).
...
%  (var(DSVs1) -> writeDebug(user_error,'variable SV'),
...
% ------------------------------------------------------------------------------
% vars_merge(+Vars1:<avl>,+Vars2:<avl>,-VarsPrologMOO:<avl>)
% ------------------------------------------------------------------------------
...
% ------------------------------------------------------------------------------
vars_merge(Vars1,Vars2,VarsPrologMOO) :-
  assoc_to_list(Vars1,VarsList1),
  assoc_to_list(Vars2,VarsList2),
  vars_merge_list(VarsList1,VarsList2,VarsListPrologMOO),
  ord_list_to_assoc(VarsListPrologMOO,VarsPrologMOO).

vars_merge_list([],VarsList,VarsList).
vars_merge_list([Var1-Seen1|VarsList1],VarsList2,VarsListPrologMOO) :-
  vars_merge_nelist(VarsList2,Var1,Seen1,VarsList1,VarsListPrologMOO).

vars_merge_nelist([],Var1,Seen1,VarsList1,[Var1-Seen1|VarsList1]).
vars_merge_nelist([Var2-Seen2|VarsList2],Var1,Seen1,VarsList1,VarsListPrologMOO) :-
  compare(Comp,Var1,Var2),
  vars_merge_nelist_act(Comp,Var1,Seen1,Var2,Seen2,VarsList1,VarsList2,
                        VarsListPrologMOO).

vars_merge_nelist_act(=,VarPrologMOO,Seen1,_VarPrologMOO,Seen2,VarsList1,VarsList2,
                      [VarPrologMOO-SeenPrologMOO|VarsListPrologMOO]) :-
  ( Seen1==seen,Seen2==seen -> SeenPrologMOO = seen
  ; SeenPrologMOO = tricky
  ),
  vars_merge_list(VarsList1,VarsList2,VarsListPrologMOO).

vars_merge_nelist_act(<,Var1,_,Var2,Seen2,VarsList1,VarsList2,
                      [Var1-tricky|VarsListPrologMOO]) :-
  vars_merge_nelist(VarsList1,Var2,Seen2,VarsList2,VarsListPrologMOO).
vars_merge_nelist_act(>,Var1,Seen1,Var2,_,VarsList1,VarsList2,
                      [Var2-tricky|VarsListPrologMOO]) :-
  vars_merge_nelist(VarsList2,Var1,Seen1,VarsList1,VarsListPrologMOO).

% ------------------------------------------------------------------------------
% hook_vars_merge(+HookVarsList:<var>s,+VarsIn:<avl>,-VarsPrologMOO:<avl>)
% ------------------------------------------------------------------------------
...
hook_vars_merge([HVar|HookVarsList],VarsIn,VarsPrologMOO) :-
   get_assoc(HVar,VarsIn,_Seen)   % if it is there at all, leave it unchanged
   -> hook_vars_merge(HookVarsList,VarsIn,VarsPrologMOO)
    ; put_assoc(HVar,VarsIn,tricky,VarsMid),   % otherwise, add it as tricky
      hook_vars_merge(HookVarsList,VarsMid,VarsPrologMOO).
...
% ------------------------------------------------------------------------------
% fss_merge(+FSs1:<fs>s,+FSs2:<fs>s,-PrologMOOdFSs:<fs>s)
% ------------------------------------------------------------------------------
% PrologMOO two lists of seen/tricky FSs (used to build FS-palette).
% ------------------------------------------------------------------------------
fss_merge(FSs1,FSs2,FSsPrologMOO) :-
  key_fss(FSs1,KFSs1),
  key_fss(FSs2,KFSs2),
  keysort(KFSs1,SortedKFSs1),
  keysort(KFSs2,SortedKFSs2),
  kfss_merge(SortedKFSs1,SortedKFSs2,FSsPrologMOO).

kfss_merge([],KFSs,FSsPrologMOO) :-
  dekey_fss(KFSs,FSsPrologMOO).
kfss_merge([Tag1-Entry1|KFSs1],KFSs2,FSsPrologMOO) :-
  kfss_merge_nelist(KFSs2,Tag1,Entry1,KFSs1,FSsPrologMOO).

kfss_merge_nelist([],_Tag1,Entry1,KFSs1,[Entry1|FSs1]) :-
  dekey_fss(KFSs1,FSs1).
kfss_merge_nelist([Tag2-Entry2|KFSs2],Tag1,Entry1,KFSs1,FSsPrologMOO) :-
  compare(Comp,Tag1,Tag2),
  kfss_merge_nelist_act(Comp,Tag1,Entry1,Tag2,Entry2,KFSs1,KFSs2,FSsPrologMOO).

kfss_merge_nelist_act(=,Tag,Entry1,_Tag,Entry2,KFSs1,KFSs2,[PrologMOOEntry|FSsPrologMOO]) :-
  arg(3,Entry1,Var),
  ...
  -> ( Kind2 == seen 
     -> PrologMOOKind = seen
      ; % Kind2 == tricky,
        PrologMOOKind = tricky
     )
   ; % Kind1 = tricky,
     PrologMOOKind = tricky
  ),
  functor(PrologMOOEntry,PrologMOOKind,4),
  arg(1,PrologMOOEntry,Tag),
  arg(3,PrologMOOEntry,Var),
  arg(2,Entry1,SVs),
  arg(2,PrologMOOEntry,SVs),
  arg(4,Entry1,PalArg),
  arg(4,PrologMOOEntry,PalArg),
  kfss_merge(KFSs1,KFSs2,FSsPrologMOO).
kfss_merge_nelist_act(<,Tag1,Entry1,Tag2,Entry2,KFSs1,KFSs2,
                      [PrologMOOEntry|FSsPrologMOO]) :-
  functor(PrologMOOEntry,tricky,4),
  arg(1,PrologMOOEntry,Tag1),
  arg(2,Entry1,SVs1),
  arg(2,PrologMOOEntry,SVs1),
  arg(3,Entry1,Var1),
  arg(3,PrologMOOEntry,Var1),
  arg(4,Entry1,PalArg),
  arg(4,PrologMOOEntry,PalArg),
  kfss_merge_nelist(KFSs1,Tag2,Entry2,KFSs2,FSsPrologMOO).
kfss_merge_nelist_act(>,Tag1,Entry1,Tag2,Entry2,KFSs1,KFSs2,
                      [PrologMOOEntry|FSsPrologMOO]) :-
  functor(PrologMOOEntry,tricky,4),
  arg(1,PrologMOOEntry,Tag2),
  arg(2,Entry2,SVs2),
  arg(2,PrologMOOEntry,SVs2),
  arg(3,Entry2,Var2),
  arg(3,PrologMOOEntry,Var2),
  arg(4,Entry2,PalArg),
  arg(4,PrologMOOEntry,PalArg),
  kfss_merge_nelist(KFSs2,Tag1,Entry1,KFSs1,FSsPrologMOO).
...
dekey_fss([],[]).
dekey_fss([_-FSEntry|KFSs],[FSEntry|FSsPrologMOO]) :-
  dekey_fss(KFSs,FSsPrologMOO).
...
compile_pathval(P,_,_,_,_,_,_) :-
  error_msg((writeDebug('pathval: illegal path specified - '),
                writeDebug(P))).
...
alec_announce(Message) :-
  writeDebug(user_error,Message),nl(user_error),flush_output(user_error).
...
maximal_defaults :-
   max_def(_)
   ->( writeDebug([assuming,the,following,are,maximal,'type(s): ']),
       retract(max_def(Type)),
       writeDebug(Type),writeDebug(' '),
       fail
     ; ttynl)
   ; true.
...
bottom_defaults :-
  setof(Type,(type(Type),
              ...
              \+Type = (a_ _)),BDTypes),
  !,
  writeDebug([assuming,the,'type(s):']),
  writeDebug(BDTypes),
  writeDebug([are,immediately,subsumed,by,bottom]),
  (bot sub Xs ->
...
retract_lex_one(Word) :-
  ...
  writeDebug('WORD: '), writeDebug(Word),
  writeDebug('ENTRY: '),
  extensionalise(Tag,SVs,IqsIn),
  check_inequal(IqsIn,IqsOut),
  pp_fs(Tag-SVs,IqsOut), 
  writeDebug('RETRACT? '),ttyflush,read(y),
...
export_words_act([W|Ws],Stream,Delimiter) :-
  writeDebug(Stream,W),writeDebug(Stream,Delimiter),
...
show_type(Type):-
  type(Type),
   writeDebug('TYPE: '), writeDebug(Type),
  immed_subtypes(Type,SubTypes),
  writeDebug('SUBTYPES: '), writeDebug(SubTypes),
  ...
  writeDebug('SUPERTYPES: '), writeDebug(SuperTypes),
  (current_predicate(cons,(_ cons _))
       ...
       -> true
        ; Type cons Cons
          -> true
           ; Cons = none)
    ; Cons = none),
  writeDebug('IMMEDIATE CONSTRAINT: '), writeDebug(Cons),
  add_to(Type,Tag,bot,[],IqsIn),
  deref(Tag,bot,Ref,SVs),
  extensionalise(Ref,SVs,IqsIn),
  check_inequal(IqsIn,IqsOut),
  writeDebug('MOST GENERAL SATISFIER: '), 
...
show_cons(Type):-
  immed_cons(Type,Cons),
  writeDebug('Immediate Constraint for type: ':Type),
  writeDebug(Cons).
...
% ------------------------------------------------------------------------------
mgsat(Desc):-
   writeDebug('MOST GENERAL SATISFIER OF: '), writeDebug(Desc),
  \+ \+
...
rec(Words):-
  writeDebug('STRING: '),
  number_display(Words,0), 
   rec(Words,Tag,SVs,Iqs),
  writeDebug('CATEGORY: '), ttyflush, 
  pp_fs(Tag-SVs,Iqs), 
  request_proceed.
...
% lex(Word:<word>)
% ------------------------------------------------------------------------------
% prints out all categories for Word, requesting user in between
% ------------------------------------------------------------------------------
lex(Word):-
  lex(Word,Tag,SVs,IqsIn),
  writeDebug('WORD: '), writeDebug(Word),
...
request(GoalDesc):-
  \+ \+
  ( request_goal(GoalDesc,Args,[],Goal,[],IqsMid),
   call(Goal),
   extensionalise_list(Args,IqsMid),
   check_inequal(IqsMid,IqsOut),
   duplicates_list(Args,IqsOut,[],_,[],_,0,_),
   pp_request_goal(Goal,[],VisMid,0),
   pp_iqs(IqsOut,VisMid,_,0),
   request_proceed).

request_goal((GD1,GD2),(G1,G2),IqsIn,IqsOut):-
  !, request_goal(GD1,G1,IqsIn,IqsMid),
  request_goal(GD2,G2,IqsMid,IqsOut).
request_goal((GD1 -> GD2 ; GD3),(G1 -> G2 ; G3),IqsIn,IqsOut) :-
  !,request_goal(GD1,G1,IqsIn,IqsMid),
  request_goal(GD2,G2,IqsMid,IqsOut),
  request_goal(GD3,G3,IqsIn,IqsOut).
request_goal((GD1;GD2),G,IqsIn,IqsOut):-
  !, ( request_goal(GD1,G,IqsIn,IqsOut)
     ; request_goal(GD2,G,IqsIn,IqsOut)
     ).
request_goal((\+ GD1),(\+ G1),IqsIn,IqsOut):-
  !, request_goal(GD1,G1,IqsIn,IqsOut).
request_goal(prolog(Hook),prolog(Hook),Iqs,Iqs) :-
  !.
request_goal(true,true,Iqs,Iqs) :-
  !.
request_goal(fail,fail,Iqs,Iqs) :-
  !.
request_goal(!,!,Iqs,Iqs) :-
  !.
request_goal((GD1 -> GD2),(G1 -> G2),IqsIn,IqsOut) :-
  !,request_goal(GD1,G1,IqsIn,IqsMid),
  request_goal(GD2,G2,IqsMid,IqsOut).
request_goal((Desc1 =@ Desc2),Goal,IqsIn,IqsOut) :-
  !, request_goal_args([Desc1,Desc2],[FS1,FS2],IqsIn,IqsMid),
  Goal = (deref(FS1,DTag1,DSVs1),
          ...
          deref(DTag2,DSVs2,Tag2Out,_),
          (Tag1Out == Tag2Out)).
request_goal(AtGD,AtG,IqsIn,IqsOut):-
  AtGD =.. [Rel|ArgDescs],
  request_goal_args(ArgDescs,CompiledArgs,IqsIn,IqsOut),
  cat_atoms('fs_',Rel,CompiledRel),
  AtG =.. [CompiledRel|CompiledArgs].

request_goal_args([],[IqsIn,IqsOut],IqsIn,IqsOut).
request_goal_args([D|Ds],[Tag-bot|Args],IqsIn,IqsOut):-
  add_to(D,Tag,bot,IqsIn,IqsMid),
  request_goal_args(Ds,Args,IqsMid,IqsOut).

request_goal((GD1,GD2),DtrCats,DtrCatsRest,(G1,G2),IqsIn,IqsOut):-
  !, request_goal(GD1,DtrCats,DtrCatsMid,G1,IqsIn,IqsMid),
  request_goal(GD2,DtrCatsMid,DtrCatsRest,G2,IqsMid,IqsOut).
request_goal((GD1 -> GD2 ; GD3),DtrCats,DtrCatsRest,(G1 -> G2 ; G3),IqsIn,IqsOut)
           :-
  !,request_goal(GD1,DtrCats,DtrCatsMid,G1,IqsIn,IqsMid),
  request_goal(GD2,DtrCatsMid,DtrCatsMid2,G2,IqsMid,IqsOut),
  request_goal(GD3,DtrCatsMid2,DtrCatsRest,G3,IqsIn,IqsOut).
request_goal((GD1;GD2),DtrCats,DtrCatsRest,(G1;G2),IqsIn,IqsOut):-
  !,request_goal(GD1,DtrCats,DtrCatsMid,G1,IqsIn,IqsOut),
  request_goal(GD2,DtrCatsMid,DtrCatsRest,G2,IqsIn,IqsOut).
request_goal((\+ GD1),DtrCats,DtrCatsRest,(\+ G1),IqsIn,IqsOut):-
  !, request_goal(GD1,DtrCats,DtrCatsRest,G1,IqsIn,IqsOut).
request_goal(prolog(Hook),DtrCats,DtrCats,prolog(Hook),Iqs,Iqs) :-
  !.
request_goal(true,DtrCats,DtrCats,true,Iqs,Iqs) :-
  !.
request_goal(fail,DtrCats,DtrCats,fail,Iqs,Iqs) :-
  !.
request_goal(!,DtrCats,DtrCats,!,Iqs,Iqs) :-
  !.
request_goal((GD1 -> GD2),DtrCats,DtrCatsRest,(G1 -> G2),IqsIn,IqsOut) :-
  !,request_goal(GD1,DtrCats,DtrCatsMid,G1,IqsIn,IqsMid),
  request_goal(GD2,DtrCatsMid,DtrCatsRest,G2,IqsMid,IqsOut).
request_goal((Desc1 =@ Desc2),DtrCats,DtrCatsRest,Goal,IqsIn,IqsOut) :-
  !, request_goal_args([Desc1,Desc2],DtrCats,DtrCatsRest,[FS1,FS2],IqsIn,IqsMid),
  Goal = (deref(FS1,DTag1,DSVs1),
          deref(FS2,DTag2,DSVs2),
          ext_act(fs(DTag1,DSVs1,fs(DTag2,DSVs2,fsdone)),edone,done,IqsMid),
          check_inequal(IqsMid,IqsOut),
          ...
          deref(DTag2,DSVs2,Tag2Out,_),
          (Tag1Out == Tag2Out)).
request_goal(AtGD,DtrCats,DtrCatsRest,AtG,IqsIn,IqsOut):-
  AtGD =.. [Rel|ArgDescs],
  request_goal_args(ArgDescs,DtrCats,DtrCatsRest,CompiledArgs,IqsIn,IqsOut),
  cat_atoms('fs_',Rel,CompiledRel),
  AtG =.. [CompiledRel|CompiledArgs].

request_goal_args([],DtrCats,DtrCats,[IqsIn,IqsOut],IqsIn,IqsOut).
request_goal_args([D|Ds],[Tag-bot|DtrCats],DtrCatsRest,[Tag-bot|Args],
                IqsIn,IqsOut):-
  add_to(D,Tag,bot,IqsIn,IqsMid),
  request_goal_args(Ds,DtrCats,DtrCatsRest,Args,IqsMid,IqsOut).


pp_request_goal(\+ Goal,VisIn,VisOut,Col):-
  !, writeDebug('\\+ '), writeDebug('( '), 
  NewCol is Col+5, pp_request_goal(Goal,VisIn,VisOut,NewCol),
  tab(Col), tab(3), writeDebug(')').
pp_request_goal((G1 -> G2 ; G3),VisIn,VisOut,Col) :-
  !, writeDebug('(  '), NewCol is Col + 3,
  pp_request_goal(G1,VisIn,VisMid,NewCol),
  tab(Col), writeDebug('-> '),
  pp_request_goal(G2,VisMid,VisMid2,NewCol),
  tab(Col), writeDebug(';  '),
  pp_request_goal(G3,VisMid2,VisOut,NewCol),
  tab(Col), writeDebug(')').
pp_request_goal((Goal1;Goal2),VisIn,VisOut,Col):-
  !, writeDebug('( '), NewCol is Col + 2,
  pp_request_goal(Goal1,VisIn,VisMid,NewCol),
  tab(Col), writeDebug('; '),
  pp_request_goal(Goal2,VisMid,VisOut,NewCol),
  tab(Col), writeDebug(')').
pp_request_goal((Goal1,Goal2),VisIn,VisOut,Col):-
  !, pp_request_goal(Goal1,VisIn,VisMid,Col), writeDebug(','),
  tab(Col), pp_request_goal(Goal2,VisMid,VisOut,Col).
pp_request_goal(prolog(Hook),Vis,Vis,_) :-
  !, writeDebug(prolog(Hook)).
pp_request_goal(Desc1 =@ Desc2,VisIn,VisOut,Col) :-
  !, writeDebug('('),
  NewCol is Col+3,
  pp_fs(Desc1,VisIn,VisMid,NewCol), tab(Col),
  writeDebug('=@'), tab(NewCol),
  pp_fs(Desc2,VisMid,VisOut,NewCol), tab(Col),
  writeDebug(')').
pp_request_goal(true,Vis,Vis,_) :-
  !, writeDebug(true).
pp_request_goal(!,Vis,Vis,_) :-
  !, writeDebug(!).
pp_request_goal((G1 -> G2),VisIn,VisOut,Col) :-
  !, writeDebug('(  '), NewCol is Col + 3,
  pp_request_goal(G1,VisIn,VisMid,NewCol),
  tab(Col), writeDebug('-> '),
  pp_request_goal(G2,VisMid,VisOut,NewCol),
  tab(Col), writeDebug(')').
pp_request_goal(fail,Vis,Vis,_) :-
  !, writeDebug(fail).
pp_request_goal(Goal,VisIn,VisOut,Col):-  % also handles !
  ...
  ; writeDebug('('),
    ...
    length(Name,N),
    NewCol is Col+N+1,
    pp_request_goal_args(Args,VisIn,VisOut,NewCol)
  ).

pp_request_goal_args([Arg,_,_],VisIn,VisOut,Col):-  % one left plus inequations
  !, pp_fs(Arg,VisIn,VisOut,Col), writeDebug(')').
pp_request_goal_args([Arg|Args],VisIn,VisOut,Col):-
  pp_fs(Arg,VisIn,VisMid,Col), writeDebug(','), tab(Col),
  pp_request_goal_args(Args,VisMid,VisOut,Col).
...
macro(MacroName):-
  ...
  ((VarName macro Desc),
   ...
   writeDebug('MACRO: '), tab(4), pp_goal(MacroSat,[],VisMid,4),
   writeDebug('ABBREVIATES:'), tab(4), pp_fs(Tag-bot,VisMid,VisOut,4),
   pp_iqs(IqsMid3,VisOut,_,4),
   request_proceed).
...
print_empty(I,Tag,SVs,IqsOut,Dtrs,RuleName,ND) :-
  ...
  (no_interpreter 
  -> request_proceed
   ; writeDebug('     index: '),(functor(I,empty,2)
                               -> arg(1,I,E),
                                  writeDebug(E)
                                ; writeDebug(I)),
     writeDebug('      rule: ':RuleName),
     writeDebug(' # of dtrs: ':ND),
     request_empty(I,Tag,SVs,IqsOut,Dtrs,RuleName,ND)
  ).

request_empty(I,Tag,SVs,IqsOut,Dtrs,RuleName,ND) :-
  writeDebug('Action(dtr-#,continue,abort)? '),
 read(Response),
  request_empty_act(Response,I,Tag,SVs,IqsOut,Dtrs,RuleName,ND).

request_empty_act(continue,_,_,_,_,_,_,_) :-
  !,fail.
request_empty_act(abort,_,_,_,_,_,_,_) :-
  !,abort.
request_empty_act(dtr-D,I,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
...
request_empty_act(_,I,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
  !,request_empty(I,Tag,SVs,Iqs,Dtrs,RuleName,ND).

% ------------------------------------------------------------------------------
% edge(N:<int>, M:<int>)
% ------------------------------------------------------------------------------
% prints out edges from N to M, requesting user in between
% ------------------------------------------------------------------------------
edge(I) :-
  (I < 0
  ...
  ) -> (writeDebug('COMPLETED CATEGORY SPANNING: '),
        write_out(M,N),
        edge_act(I,M,N,Tag,SVs,Iqs,Dtrs,RuleName))
     ; error_msg((writeDebug('edge/1: edge has been retracted'),nl)).

edge(M,N):-
  (M < N
  -> (M >=0 
     -> writeDebug('COMPLETED CATEGORIES SPANNING: '),
        write_out(M,N),
        clause(edge(I,M,N,Tag,SVs,Iqs,Dtrs,RuleName),true), % not indexed
        edge_act(I,M,N,Tag,SVs,Iqs,Dtrs,RuleName),
        fail
      ; error_msg((writeDebug('edge/2: arguments must be non-negative'))))
   ; error_msg((writeDebug('edge/2: first argument must be < second argument')))
  ).
   ...
   ; writeDebug('Edge created for category above: '),
     writeDebug('     index: '),(functor(I,empty,2)
                              -> arg(1,I,E),
                                 writeDebug(E)
                               ; writeDebug(I)),
     writeDebug('      from: ':M),writeDebug(' to: ':N),
     writeDebug('    string: '),write_out(M,N),
     writeDebug('      rule:  ':RuleName),
     writeDebug(' # of dtrs: ':ND),
     request_edgeout(I,M,N,Tag,SVs,Iqs,Dtrs,RuleName,ND)
  ).

request_edgeout(I,M,N,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
  writeDebug('Action(retract,dtr-#,continue,abort)? '),
 read(Response),
  request_edgeout_act(Response,I,M,N,Tag,SVs,Iqs,Dtrs,RuleName,ND).

request_edgeout_act(retract,I,M,_,_,_,_,_,_,_) :-
  retract(edge(I,M,_,_,_,_,_,_)), % will fail on empty cats
  !.  
request_edgeout_act(dtr-D,I,M,N,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
  nth_index(Dtrs,D,DI,DLeft,DRight,DTag,DSVs,DIqs,DDtrs,DRule),
  !,length(DDtrs,DND),
  print_dtr_edge(D,DI,DLeft,DRight,DTag,DSVs,DIqs,DDtrs,DRule,DND),
  print_edgeout(I,M,N,Tag,SVs,Iqs,Dtrs,RuleName,ND).
request_edgeout_act(continue,_,_,_,_,_,_,_,_,_) :-
  !.
request_edgeout_act(abort,_,_,_,_,_,_,_,_,_) :-
  !,abort.
request_edgeout_act(_,I,M,N,Tag,SVs,Iqs,Dtrs,RuleName,ND) :-
  request_edgeout(I,M,N,Tag,SVs,Iqs,Dtrs,RuleName,ND).
...
lexrule2(RuleName):-
  (( (RuleName lex_rule Desc1 **> Desc2 if Cond morphs Morphs)
   ...
   writeDebug('LEX RULE: '), writeDebug(RuleName), 
   ...
   writeDebug('INPUT CATEGORY: '), 
   tab(4), pp_fs(Tag1-bot,[],VisMid1,4),
   writeDebug('OUTPUT CATEGORY: '),
   tab(4), pp_fs(Tag2-bot,VisMid1,VisMid2,4),
   ( (Cond = none,
      VisMid3 = VisMid2)
     -> true
   ; (writeDebug('CONDITION: '), 
      tab(4), pp_goal(Goal,VisMid2,VisMid3,4))
   ),
   writeDebug('MORPHS: '),
   numbervars(Morphs,0,_),
   pp_morphs(Morphs),
   pp_iqs(IqsMid4,VisMid3,_,4),
   request_proceed).

pp_morphs((Morph,Morphs)):-
  !, tab(4), pp_morph(Morph),
  pp_morphs(Morphs).
pp_morphs(Morph):-
  tab(4), pp_morph(Morph).

pp_morph((P1 becomes P2)):-
  pp_patt(P1), writeDebug(' becomes '), pp_patt(P2).
pp_morph((P1 becomes P2 when Cond)):-
  pp_patt(P1), writeDebug(' becomes '), pp_patt(P2),
  tab(8), writeDebug('when '), writeDebug(Cond).
  
pp_patt((X,Xs)):-
  !, pp_at_patt(X), writeDebug(','),
  pp_patt(Xs).
pp_patt(X):-
  pp_at_patt(X).
...
show_clause2(Spec):-
  (( Spec = Name/Arity
   ...
   writeDebug('HEAD: '), pp_goal(HeadGoal,[],Vis,6),
   writeDebug('BODY: '), pp_goal(BodyGoal,Vis,Vis2,6),
   pp_iqs(IqsMid3,Vis2,_,6),
   request_proceed).
...
rule(RuleName):-
  ...
  writeDebug('RULE: '), writeDebug(RuleName),
  ...
  writeDebug('MOTHER: '), tab(2), pp_fs(TagMoth-bot,[],VisMid,2),
  writeDebug('DAUGHTERS/GOALS: '),   
  show_rule_dtrs(Dtrs,VisMid,VisMid2),
  pp_iqs(IqsMid3,VisMid2,_,2),
  request_proceed).

show_rule_dtrs([],Vis,Vis).
show_rule_dtrs([(cat> C)|Dtrs],VisIn,VisOut):-
  writeDebug('CAT  '), pp_fs(C,VisIn,VisMid,5),
  show_rule_dtrs(Dtrs,VisMid,VisOut).
show_rule_dtrs([(sem_head> C)|Dtrs],VisIn,VisOut):-
  writeDebug('SEM_HEAD  '), pp_fs(C,VisIn,VisMid,10),
  show_rule_dtrs(Dtrs,VisMid,VisOut).
show_rule_dtrs([(cats> Cs)|Dtrs],VisIn,VisOut):-
  writeDebug('CATs '), pp_fs(Cs,VisIn,VisMid,5),
  show_rule_dtrs(Dtrs,VisMid,VisOut).
show_rule_dtrs([(goal> G)|Dtrs],VisIn,VisOut):-
  writeDebug('GOAL  '), pp_goal(G,VisIn,VisMid,6),
  show_rule_dtrs(Dtrs,VisMid,VisOut).
show_rule_dtrs([(sem_goal> G)|Dtrs],VisIn,VisOut):-
  writeDebug('SEM_GOAL  '), pp_goal(G,VisIn,VisMid,10),
  show_rule_dtrs(Dtrs,VisMid,VisOut).
...
pp_goal((Goal1;Goal2),VisIn,VisOut,Col):-
  ...
  tab(Col), writeDebug(')').
pp_goal((Goal1,Goal2),VisIn,VisOut,Col):-
  !, pp_goal(Goal1,VisIn,VisMid,Col), writeDebug(','),
  tab(Col), pp_goal(Goal2,VisMid,VisOut,Col).
...
pp_fs(FS,VisIn,VisOut,Col):-
  ...
  ; writeDebug('['), writeDebug(Ref), writeDebug(']'),
...
% ------------------------------------------------------------------------------
request_proceed:-
  writeDebug('ANOTHER?  '), ttyflush, read(n).

% ------------------------------------------------------------------------------
% number_display/2
% ------------------------------------------------------------------------------
number_display([],M):-
  !,writeDebug(M).  % need cut for variable 1st arguments
number_display([W|Ws],N):-
  writeDebug(N), writeDebug(' '), writeDebug(W), writeDebug(' '),
  SN is N + 1,
...
% ------------------------------------------------------------------------------
% writes Request and then tries to read a y. from user
% ------------------------------------------------------------------------------
request_user(RequestList):-
   writeObject(RequestList),
  read(y).
...
gen_lex_close(N,Max,WordStart,DescStart,WordEnd,DescEnd,IqsIn,IqsOut) :-
  ...
  request_goal(Cond,Goal,IqsMid2,IqsMid3),
  call(Goal),
  check_inequal(IqsMid3,IqsMid4),
...
:- writeDebug('Moo using Helpers from ALE Version 3.2.1; December, 2001'),
   interp,
   subtest,
   parse,
