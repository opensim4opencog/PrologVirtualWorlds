...
public class PlayerMask extends PartiallyTangible implements JamudEventTrigger {
   ...
   public static final String[] TRUST = {
	...
	public void setShortName(String name) {
		...
		if(triggerPlayerMaskEvents())
			Jamud.getEventManager().triggerListeners(new EngineerMaskEvent(this, EngineerMaskEvent.EVENT_UPDATE, new Object[0]));
	}
	...
	public Gender getGender() {
		if(isLocal()) {
			AgentGeneric b = (AgentGeneric) player.getProperty(Engineer.PROPERTY_NATIVE_BODY);
			if ( b.getGender() == null )
	...
	public int getLevel() {
		if(isLocal()) {
			AgentGeneric b = (AgentGeneric) player.getProperty(Engineer.PROPERTY_NATIVE_BODY);
			return b.getStats().stat(AgentGeneric.STAT_LEVEL, 1);
		}
		else return level;
	}
	...
	public int getVisible() {
		if(isLocal())
			return ((AgentGeneric) player.getProperty(Engineer.PROPERTY_NATIVE_BODY)).getStats().stat(AgentGeneric.STAT_VISIBLE, 0);
		else
			return vis;
	}
