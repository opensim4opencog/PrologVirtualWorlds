...
public class PlayerManager implements ServerListener, JamudEventTrigger {
   ...
   private String path;
	...
	public void triggerPlayerLogIn(Engineer p) {
		//first, add the player for our use
		this.put(p);

		//trigger registered listeners
		Object[] o = { p };
		Jamud.getEventManager().triggerListeners(new EngineerEvent(this, EngineerEvent.EVENT_LOGIN, o));
	}


	public void triggerPlayerLogOut(Engineer p) {
		//first, remove him from us
		this.remove(p.getName());

		//trigger registered listeners (for us)
		Object[] o = { p };
		Jamud.getEventManager().triggerListeners(new EngineerEvent(this, EngineerEvent.EVENT_LOGOUT, o));
	}


   public void localMudOut(String text) {
      for(Enumeration j = elements(); j.hasMoreElements(); ) {
         Engineer nxt = ((PlayerMask) j.nextElement()).getEngineer();
	...
	public Engineer findPlayer(String playerName) {
		if(playerName == null || playerName.length() == 0)
			return null;
		...
		if(pl == null)
			try {
				...
				xmp = null;
				return Engineer.loadPlayer(xml);
			}
			catch(Exception e) {
				return null;
			}
		else
			return pl.getEngineer();
	}
