...
relevant_pos_lit(Ggoal,GH,Val,Tab,Plits0,Plits,Nlits0,Nlits,Nseq0,Nseq) :-
	( slg_find(Plits0,GH,v(Val0,_,_)) ->
	  Val0 = Val, Plits = Plits0, Nlits = Nlits0, Nseq = Nseq0
        ; slg_find(Tab,Ggoal,Ent),
	  ent_to_anss(Ent,Anss),
	  slg_find(Anss,GH,Lanss),
	  relevant_anshead(GH,Lanss,Val,Tab,Plits0,Plits,Nlits0,Nlits,Nseq0,Nseq)
        ).
...
assume_one(Lit,Val,Tab0,Tab,Plits) :-
	...
	slg_find(Plits,GH,v(Val0,_Con,_)),
	( stableVar(Val0) ->
	  Val0 = AVal, Tab = Tab0
        ; Val0 = AVal,
...
construct_psm(l(_,v(Val,Con,H)),Nlits,Tab0,Tab,PSM0,PSM) :-
	( Con == true -> % confirmed with the table
	  ...
	  ( stableGround(H), slg_find(Nlits,H,Val) ->
	    PSM0 = [ /*neg*/ \+ H|PSM]
	  ; PSM0 = PSM
          )
        ).
...
simplify_one(Val,Link,Tab0,Tab,Plits) :-
    ...
    ( updatevs(Anss0,GH,Lanss0,Lanss,Anss) ->
      ...
      ( stableVar(DVal) ->
        ( slg_find(Plits,GH,v(AVal,Con,_)) ->
...
slg_find(Tree,Key,V) finds the entry with Key : returns associated
    values in V.
...
slg_find(l(X,V),Xs,V) :- X == Xs.
slg_find(n2(T1,M,T2),X,V) :-
	M @=< X
	 ->	slg_find(T2,X,V)
	 ;	slg_find(T1,X,V).
slg_find(n3(T1,M2,T2,M3,T3),X,V) :-
	M2 @=< X
	 ->	(M3 @=< X
		 ->	slg_find(T3,X,V)
		 ;	slg_find(T2,X,V)
...
%      (I am interested to hear about them if you slg_find them).
...
the same as \+slg(Call). Channel check if Call is false (according to the
