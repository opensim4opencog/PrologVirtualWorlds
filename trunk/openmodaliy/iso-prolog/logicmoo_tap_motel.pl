...
loadLibraries(yap) :-
	...
	assertz((motel_retractall(Head) :- retract(Head), fail)),
	assertz((motel_retractall(Head) :- retract((Head :- _Body)), fail)),
	assertz((motel_retractall(_))),
	assertz((onceOrMore(Goal) :- option(allProofs,yes), Goal)),
	assertz((onceOrMore(Goal) :- not(option(allProofs,yes)), Goal, !)).
...
getLibraries :-
	catch(current_op(_X,hy,_Y),_,fail),
	loadLibraries(xsbprolog),
	!.


getLibraries :-
	catch(yap_flag(_X,_Y),_,fail),
	!,
	asserta(currentProlog(swiprolog)),
	loadLibraries(yap).
...
setOption(Option,Set) :-
	motel_retractall(option(Option,_)),
...
motel_retractall(Env,Pred/Arity) :-
	constructHead(Env,Pred/Arity,Head),
	motel_retractall(Head), 
	!.
...
%   motel_intersection(+Set1, +Set2, ?Intersection)
...
%   the motel_intersection of the sets represented by Set1 and Set2.
...
motel_intersection([], _, []).
motel_intersection([Element|Elements], Set, Intersection) :-
	motel_memberchk(Element, Set),
	!,
	Intersection = [Element|Rest],
	motel_intersection(Elements, Set, Rest).
motel_intersection([_|Elements], Set, Intersection) :-
	motel_intersection(Elements, Set, Intersection).



%   motel_intersection(+ListOfSets, ?Intersection)
%   is true when Intersection is the motel_intersection of all the sets in
...
motel_intersection([Set|Sets], Intersection) :-
...
motel_list_to_set([], []).
motel_list_to_set([Head|Tail], Set) :-
	motel_memberchk(Head, Tail),
	!,
	motel_list_to_set(Tail, Set).
motel_list_to_set([Head|Tail], [Head|Set]) :-
	motel_list_to_set(Tail, Set).
...
%   Channel extract a single copy of Kill, use select(Kill, List, Residue).
...
% Channel prove in(C,X) it is not allowed to use another in-clause generated 
% from the same axiom
...
sameAxiom(AX,_RN1,MS1,in(_C,X1),HYPS1,_D1,
	...
	testEqualHypotheses(HYPS1,HYPS2),
	equalWorlds(MS1,MS2), !.
% Channel prove in(C,X) it is not allowed to use a constraint-clause generated 
% from the same axiom
sameAxiom(AX,_RN1,MS1,in(_C,X1),HYPS1,_D1,
...
init_succ(_MS).
init_succ(Env,MS) :- 
	motel_retractall(succ(_,Env,MS,_,_)),
	!.
...
init_sub(_MS).
init_sub(Env,MS) :- 
	motel_retractall(sub(_,Env,MS,_,_)),
	!.
...
init_nsub(_MS).
init_nsub(Env,MS) :-
	motel_retractall(nsub(_,Env,MS,_,_)),
	!.
...
newClassify(EnvName,MS) :-
	...
	motel_retractall(conceptHierarchy(Env,MS,_)),
	motel_retractall(roleHierarchy(Env,MS,_)),
...
subsume1(Type,Env,MS,X,Y) :- 
	X \== Y,
	addCounter(Type,1),
	ifOption(testOutput,yes,(getRuntime(T1),format('~3d: ~a >= ~a',[T1,X,Y]))),
	subsumes(Type,Env,MS,X,Y), 
	ifOption(testOutput,yes,(getRuntime(T1),format(' succeeded (~3d)~n',[T1]))),
	cont(Type,Env,MS,[],X,Y),
	!.
subsume1(Type,Env,MS,X,Y) :- 
	X \== Y,
	ifOption(testOutput,yes,(getRuntime(T1),format(' failed (~3d)~n',[T1]))),
...
%	write((:- dynamic(rel/5))), write('.'), nl,
	...
	writeq((:- motel_retractall(currentEnvironment(_)))), write('.'), nl,
...
%	SolveHead = solveConstraint(AN,MS,(card,app((FF:R),X),Rel,N),hyp(HYPS)),
	length(CALLS,XXX),
	CALLS1 = CALLS,
	ifOption(traceOutput,yes,(format('trying ~d  solve(~w(~w)) ~w ~w~n',[XXX,R,X,Rel,N]))),
...
makeEnvironment(Name,Comment) :-
	...
	motel_retractall(currentEnvironment(_)),
...
removeEnvironment(Name) :-
	...
	motel_retractall(environment(Name,_,_)),
...
clearEnvironment(EnvName) :-
	...
	motel_retractall(Env,in/9),
	motel_retractall(Env,kb_in/10),
	motel_retractall(Env,eq/9),
	motel_retractall(Env,constraint/8),
	motel_retractall(Env,rel/5),
	motel_retractall(Env,closed/5),
	motel_retractall(Env,compiledPredicate/2),
	motel_retractall(Env,conceptElement/7),
	motel_retractall(Env,conceptEqualSets/6),
	motel_retractall(Env,conceptHierarchy/3),
	motel_retractall(Env,conceptName/4),
	motel_retractall(Env,conceptSubsets/6),
	motel_retractall(Env,environment/3),
	motel_retractall(Env,given_change/4),
	motel_retractall(Env,given_inflLink/4),
	motel_retractall(Env,modalAxioms/7),
	motel_retractall(Env,roleAttributes/5),
	motel_retractall(Env,roleDefault/4),
	motel_retractall(Env,roleDefNr/4),
	motel_retractall(Env,roleDomain/4),
	motel_retractall(Env,roleElement/8),
	motel_retractall(Env,roleEqualSets/6),
	motel_retractall(Env,roleHierarchy/3),
	motel_retractall(Env,roleName/4),
	motel_retractall(Env,roleNr/5),
	motel_retractall(Env,roleRange/4),
	motel_retractall(Env,roleSubsets/6),
	motel_retractall(sub(_,Env,_,_,_)),
	motel_retractall(succ(_,Env,_,_,_)),
	motel_retractall(Env,abductiveDerivation/3),
	motel_retractall(Env,consistencyDerivation/3),
	motel_retractall(Env,hypothesis/1),
	motel_retractall(Env,inconsistencyCheck/3),
	motel_retractall(Env,option/2),
	motel_retractall(nsub(_,Env,_,_,_)),
	motel_retractall(Env,nsub3/2),
	motel_retractall(Env,sub3/2),
	motel_retractall(Env,succ3/2),
	!.
...
assertInRule(Env,1) :-
	...
	assertz((InHead :- ground_copy(X,X))),
	!.
assertInRule(Env,2) :-
...
assertInRule(Env,8,AN7) :-
	...
	%            motel_intersection(S2,S3,S1) => X in S1
	% Priority 1 (low priority)
	gensym(rule,RN8),
	constructKBHead(Env,pr(1),rn(AN7,RN8,system,lInR),MS,set(S1),X,
	...
	L1 = motel_intersection([S2,S3],S1),
	constructMLMark(Consequence1,AxiomHead1),
...
% should be a motel_subset of the results for
...
logic_subst(T1,T2,X,Sk) :-
	(atomic(T1) ; var(T1)),
	T1 == X,
	!,
	T2 = Sk.
logic_subst(T1,T2,X,_Sk) :-
	(atomic(T1) ; var(T1)),
	not(T1 == X),
	!,
	T2 = T1.
logic_subst(forall(Y,P),forall(Y,P),X,_Sk) :-
	X == Y,
	!.
logic_subst(forall(Y,P),forall(Y,P1),X,Sk) :-
	!,
	logic_subst(P,P1,X,Sk).
logic_subst(exists(Y,P),exists(Y,P),X,_Sk) :-
	X == Y,
	!.
logic_subst(exists(Y,P),exists(Y,P1),X,Sk) :-
	!,
	logic_subst(P,P1,X,Sk).
logic_subst(and(L),and(L1),X,Sk) :-
	!,
	map(logic_subst,[X,Sk],L,L1).
logic_subst(or(L),or(L1),X,Sk) :-
	!,
	map(logic_subst,[X,Sk],L,L1).
logic_subst(not(P),not(P1),X,Sk) :-
	!,
	logic_subst(P,P1,X,Sk).
logic_subst(T1,T2,X,Sk) :-
	!,
	T1 =.. [F|Args],
	map(logic_subst,[X,Sk],Args,Args1),
	T2 =.. [F|Args1].
...
assert_ind(EnvName,MS,X,C) :-
	...
	defineEquivalentConceptname(EnvName,MS,C,assert,C1),
	convertMS(positive,Env,[[],true],MS,[],[W1,G1],_),
	asserta((conceptElement(Env,MS,W1,user,X,C1,AxiomName) :- call(G1))),
	assertz(axiom(Env,MS,assert_ind(MS,X,C1))),
...
assert_ind(EnvName,MS,X,Y,RT) :-
	...
	atomic(X),
	atomic(Y),
	defineEquivalentRolename(EnvName,MS,RT,assert,R),
	Role1 =.. [R,X,Y],
	asserta(Role1),
%	Role2 =.. [R,X,Y],
...
envToFOL(Name,CL) :-
	theory(Name,CLI0),
	map(translateFOLtoClause,CLI0,CLI1), %trace,
...
modalAxioms(EnvName,MS,k,MOp,A1) :-
	...
	motel_retractall(rel(Env,C,m(MOp,A),_,_)),
	motel_retractall(modalAxioms(Env,MS,user,_,A1,MOp,A)),
	assertMA(A1,
...
modInference(CL1,Bool,CL,stat(N3,N4)) :-
	...
	ifOption(workingOutput,yes,(length(CL2,N2), write('Starting with '), print(N2), write(' clauses: '),nl,printClauseList(CL2),nl)),
...
modInference(Old,New,Result,Bool,Level) :-
	ifOption(workingOutput,yes,(write('Starting modInference on level '),print(Level),nl)),
	ifOption(compare,yes,(print('Clauses: '),nl,printClauseList(New))),
	modConsequences(product,Old,New,C1),
	modConsequences(combination,New,New,C2),
	append(Old,New,OldClauses),
	append(C1,C2,NewClauses),
%	ifOption(workingOutput,yes,(length(NewClauses,N1), write('Consequences: '),print(N1),nl)),
	ifOption(traceOutput,yes,(write('Starting subsumption tests.'),nl,write('Old clauses: '),printClauseList(OldClauses),nl,write('New clauses: '),printClauseList(NewClauses),nl)),
...
modInferenceCont(RemainingOldClauses,[],RemainingOldClauses,true,_) :-
	ifOption(compare,yes,(print('Final Clauses: '),nl,printClauseList(RemainingOldClauses))).
modInferenceCont(RemainingOldClauses,NonSubsumedClauses,Result,Bool,Level) :-
	NonSubsumedClauses \== [],
	ifOption(traceOutput,yes,(length(RemainingOldClauses,N2), write('Remaining old clauses: '),print(N2),nl,printClauseList(RemainingOldClauses),nl)),
        ifOption(traceOutput,yes,(length(NonSubsumedClauses,N3),write('Remaining new clauses: '),print(N3),nl,printClauseList(NonSubsumedClauses),nl)),
	M is Level+1,
...
modSuccList([CT|CR],EnvName,MS,CList,SuccList) :-	
	...
	motel_union(SuccList1,SuccList2,SuccList),
	!.
...
modConsequences(Type,LList,RList,Consequences) :-
	ifOption(traceOutput,yes,(write('Starting modConsequences.'),nl)),
	ifOption(traceOutput,yes,(length(LList,N1), length(RList,N2), write('Starting outer loop with '),print(N2),write(' on '), print(N1),write(' clauses.'),nl)),
	modSetfactor(LList,Factors),
	ifOption(traceOutput,yes,(length(Factors,N3), write('Number of factors '),print(N3),nl)),
	...
	ifOption(traceOutput,yes,(length(Resolvents,N4), write('Number of resolvents '), print(N4),nl)),
...
all_maxlits_all_clauses([Clause|List],[Clause-MaxLits|ListMax]) :-
	...
	ifOption(traceOutput,yes,(write('Maximal literals '), print(MaxLits), write(' in '), print(Clause), nl)),
...
modSetproduct0(_,[], _, []) :-
	ifOption(traceOutput,yes,(nl,write('Ended outer loop.'),nl)).
modSetproduct0(product,[Head-MLitsHead|Tail], Set, Result) :-
%	ifOption(traceOutput,yes,(length(Set,N),write('Starting inner loop for '),print(Head),write(' with '),print(N),nl)),
...
modSetproduct0(combination,[Head-MLitsHead|Tail], [C1|Set], Result) :-
	ifOption(workingOutput,yes,(write('Computing all resolvents with '),print(Head),nl)),
...
modSetproduct([], _X, _, []).
%	ifOption(traceOutput,yes,(nl,write('Ended inner loop for '),print(X),nl)).
...
%	ifOption(traceOutput,yes,(write('B('),ttyflush)),
...
modSubsumes(C,D) :-
%	ifOption(traceOutput,yes,(write('Test if '), print(C), write(' subsumes '), print(D), nl)),
	modNewArgOfClause(C,[PTC,NTC],Skol1),  % ?
	modNewArgOfClause(D,[PTD,NTD],Skol2),  % ?
	motel_subset(PTC,PTD),motel_subset(NTC,NTD),  % ?
	motel_subset(Skol1,Skol2),  % ?
...
%	copy_term(cl(PC,NC),cl(PC0,NC0)),
	...
	ifOption(traceOutput,yes,(write('Ground Term D: '),print(D), nl)),
	!,
	not(not((newStillman(PC0,PD0),newStillman(NC0,ND0)))),
	ifOption(traceOutput,yes,(write('Answer is positive.'), nl)).
...
%	motel_subset(PosL2,PosL1), motel_subset(NegL2,NegL1),  % ?
%	motel_subset(Skol2,Skol1),  % ?
%	!,  % ?
	modSubsumes(Clause2,Clause1).
	ifOption(traceOutput,yes,(write('Subsumption:'),nl,print(Clause2),write(' subsumes '),nl,print(Clause1),nl)).
...
%	ifOption(traceOutput,yes,(write('Test if '), print(cl(P1,N1)), write(' subsumes '), print(cl(P2,N2)), nl)),
%	write('?'),ttyflush,
...
% Channel generate a fresh constant, the integer NUMBER is used and incremented
...
%	ifOption(traceOutput,yes,(write('Trying to remove '),print(cl(PL,NL,PT)))),
	member(P,PL),
	member(N,NL),
	not(not(modVariant(P,N))),
	!,
%	ifOption(traceOutput,yes,(write(' succeeded.'),nl)),
	modRemoveTaut(CL,CLN).
modRemoveTaut([CL1|CL],[CL1|CLN]) :-
	!,
%	ifOption(traceOutput,yes,(write(' failed.'),nl)),
	modRemoveTaut(CL,CLN).
...
%	modUnify(L11,L21),
	...
	ifOption(traceOutput,yes,(write('Resolved '), print(CL1), nl, write(' and '),print(CL2),nl,write(' on '), print(L1), write(' and '), print(L2), nl, write('getting '), print(cl(E1,E2,char(PT3,[CLEP,CLEN],Ar2))),nl)).
...
modCl_list_to_set([], []) :-
	ifOption(traceOutput,yes,(print('modCl_list_to_set done.'),nl)).
modCl_list_to_set([Head|Tail], Set) :-
	ifOption(traceOutput,yes,(length([Head|Tail],N),print(N),print(' '))),
...
%	ifOption(traceOutput,yes,(length([Element|Elements],N),write('Trying to motel_subtract '),print(Element),write(' with '),print(N))),
...
undefconcept(EnvName,MS,CN,CT) :-
	...
	motel_retractall(axiom(Env,MS,defconcept(MS,CN,CT))),
 	motel_retractall(kb_in(Env,_,rn(AX1,_,_,_),_,_,_,_,_,_,proved(in([],Name,_),_,_))),
 	retract_all(query(Env,MS,CN,_CT,_PT,_)),
	motel_retractall(eq(Env,rn(AX1,_,_,_),_,_,_,_,_,_)),
	motel_retractall(eq(Env,rn(AX1,_,_,_),_,_,_,_,_,_,_)),
	motel_retractall(constraint(Env,rn(AX1,_,_,_),_,_,_,_,_)),
	motel_retractall(constraint(Env,rn(AX1,_,_,_),_,_,_,_,_,_)),
	change_classifier(EnvName,MS,CN,CT),
	retract(conceptEqualSets(Env,_user,MS,CN,CT,AX1)),
	conceptEqualSets(Env,_,MS,_,not(CN),AX2),
 	motel_retractall(kb_in(Env,_,rn(AX2,_,_,_),_,_,_,_,_,_,proved(in([],Name,_),_,_))),
 	retract_all(query(Env,MS,CN,_CT,_PT,_)),
	motel_retractall(eq(Env,rn(AX2,_,_,_),_,_,_,_,_,_)),
	motel_retractall(eq(Env,rn(AX2,_,_,_),_,_,_,_,_,_,_)),
	motel_retractall(constraint(Env,rn(AX2,_,_,_),_,_,_,_,_)),
	motel_retractall(constraint(Env,rn(AX2,_,_,_),_,_,_,_,_,_)),
	!.

undefConcept(Env,MS,CN) :-
	conceptEqualSets(Env,user,_,CN,_,AX),
	motel_retractall(axiom(Env,MS,defconcept(MS,CN,_CT1))),
 	motel_retractall(kb_in(Env,_,rn(AX,_,_,_),_,_,_,_,_,_,proved(in([],_,_),_,_))),
   	retract_all(query(Env,MS,CN,_CT,_PT,_)),
	motel_retractall(eq(Env,rn(AX,_,_,_),_,_,_,_,_,_)),
	motel_retractall(eq(Env,rn(AX,_,_,_),_,_,_,_,_,_,_)),
	motel_retractall(constraint(Env,rn(AX,_,_,_),_,_,_,_,_)),
	motel_retractall(constraint(Env,rn(AX,_,_,_),_,_,_,_,_,_)),
	change_classifier(EnvName,MS,CN,CT),
	motel_retractall(conceptEqualSets(Env,user,MS,CN,_CT,AX)),
	fail,
	!.
undefConcept(Env,MS,CN) :-
	conceptEqualSets(Env,system,_,_,not(CN),AX),
	motel_retractall(axiom(Env,MS,defconcept(MS,not(CN),_CT1))),
 	motel_retractall(kb_in(Env,_,rn(AX,_,_,_),_,_,_,_,_,_,proved(in([],_,_),_,_))),
	motel_retractall(eq(Env,rn(AX,_,_,_),_,_,_,_,_,_)),
	motel_retractall(eq(Env,rn(AX,_,_,_),_,_,_,_,_,_,_)),
	motel_retractall(constraint(Env,rn(AX,_,_,_),_,_,_,_,_)),
	motel_retractall(constraint(Env,rn(AX,_,_,_),_,_,_,_,_,_)),
	motel_retractall(conceptEqualSets(Env,system,MS,_,_,AX)),
...
retract_all(kb_in(Env,_Name1,rn(AX,_,_,_),_,_,_,_,_,_,_)) :-
	...
	motel_retractall(kb_in(Env,rn(AX,_,_,_),_,_,_,_,_,_,proved(in([],Name,_),_,_))),
	fail.
retract_all(kb_in(Env,_Name1,rn(AX,_,_,_),_,_,_,_,_,_,_)).
...
retract_all(in(Env,_Name1,rn(AX,_,_,_),_,_,_,_,_,_,_)) :-
	...
	motel_retractall(in(Env,rn(AX,_,_,_),_,_,_,_,_,_,proved(in([],Name,_),_,_))),
	fail.
retract_all(in(Env,_Name1,rn(AX,_,_,_),_,_,_,_,_,_,_)) :-
	motel_retractall(in(Env,_Name1,rn(AX,_,_,_),_,_,_,_,_,_,_)),
	motel_retractall(in(_Name1,rn(AX,_,_,_),_,_,_,_,_,_,_)).

retract_all(query(Env,MS,CN,_CT,_PT,_PT1)) :-
	query(Env,MS,CN1,CT,PT,PT1),
	collect(PT,Liste),
	member(CN,Liste),
	motel_retractall(query(Env,MS,CN1,CT,PT,PT1)),
	fail.
retract_all(query(Env,MS,CN,_CT,_PT,_PT1)).
...
undefrole(EnvName,MS,RN,RT) :-
	...
	motel_retractall(axiom(Env,MS,defrole(MS,RN,RT))),
 	motel_retractall(kb_in(Env,_,rn(AX,_,_,_),_,_,_,_,_,_,proved(in([],Name,_),_,_))),
...
undefRole(Env,MS,RN) :-
	...
	motel_retractall(axiom(Env,MS,defrole(MS,RN,_RT1))),
 	motel_retractall(kb_in(Env,_,rn(AX,_,_,_),_,_,_,_,_,_,proved(in([],Name,_),_,_))),
	motel_retractall(eq(Env,rn(AX,_,_,_),_,_,_,_,_,_)),
	motel_retractall(eq(Env,rn(AX,_,_,_),_,_,_,_,_,_,_)),
	motel_retractall(constraint(Env,rn(AX,_,_,_),_,_,_,_,_)),
	...
	motel_retractall(roleEqualSets(Env,user,MS,RN,_RT,AX)),
	fail,
	!.
...
undefprimconcept(EnvName,MS,CN,CT) :-	
	...
	motel_retractall(axiom(Env,MS,defprimconcept(MS,CN,CT))),
 	motel_retractall(kb_in(Env,_Name1,rn(AX,_,_,_),_,_,_,_,_,_,proved(in([],Name,_),_,_))),
 	retract_all(query(Env,MS,CN,_CT,_PT,_)),
	motel_retractall(eq(Env,rn(AX,_,_,_),_,_,_,_,_,_)),
	motel_retractall(eq(Env,rn(AX,_,_,_),_,_,_,_,_,_,_)),
	motel_retractall(constraint(Env,rn(AX,_,_,lInR),_,_,_,_,_)),
	motel_retractall(constraint(Env,rn(AX,_,_,lInR),_,_,_,_,_,_)),
	change_classifier(EnvName,MS,CN,CT),
...
undefprimrole(EnvName,MS,RN,RT) :-
	...
	motel_retractall(axiom(Env,MS,defprimrole(MS,RN,RT))),
 	motel_retractall(kb_in(Env,_,rn(AX,_,_,_),_,_,_,_,_,_,proved(in([],Name,_),_,_))),
...
undefprimRole(Env,MS,RN) :-
	...
	motel_retractall(axiom(Env,MS,defprimrole(MS,RN,_RT1))),
 	retract_all(query(Env,MS,RN,_RT,_PT,_)),
 	motel_retractall(kb_in(Env,_,rn(AX,_,_,_),_,_,_,_,_,_,proved(in([],Name,_),_,_))),
	motel_retractall(eq(Env,rn(AX,_,_,_),_,_,_,_,_,_)),
	motel_retractall(eq(Env,rn(AX,_,_,_),_,_,_,_,_,_,_)),
	...
	motel_retractall(constraint(Env,rn(AX,_,_,_),_,_,_,_,_,_)),
	change_classifier(EnvName,MS,RN,_),
	motel_retractall(roleSubsets(Env,user,MS,RN,_RT,AX)),
	fail,
	!.
...
delete_ind(EnvName,MS,X,C) :-
	...
	!,
	atomic(X),
	motel_retractall(axiom(Env,MS,assert_ind(MS,X,C))),
...
%	 motel_retractall(((InHead :- call(user:G1)))),
%	 motel_retractall(((InHead :- call(G1)))),
%	 motel_retractall(((conceptElement(Env,_,W1,_,X,C,_) :- call(user:G1)))),
%	 motel_retractall(((conceptElement(Env,_,W1,_,X,C,_) :- call(G1)))).
delete_ind(P1,X,Y,R) :-
...
delete_ind(EnvName,MS,X,Y,R) :-
	...
	atomic(Y),
	atomic(R),
	motel_retractall(axiom(Env,MS,assert_ind(MS,X,Y,R))),
	Role1 =.. [R,X,Y],
	retract(Role1),
	convertMS(positive,Env,[[],true],MS,[],[W1,G1],_),
...
%	motel_retractall((EqLiteral :- (cCS(CALLS,true), call(user:G1)))),
%	motel_retractall((EqLiteral :- (cCS(CALLS,true), call(G1)))),
%	motel_retractall((roleElement(Env,_,W1,X,Y,R,_) :- call(user:G1))),
%	motel_retractall((roleElement(Env,_,W1,X,Y,R,_) :- call(G1))).
...
delete_hierarchy(Type,Env,MS,CR) :-
	...
	fail.
delete_hierarchy(Type,Env,MS,CR) :-
	motel_retractall(succ(Type,Env,MS,CR,_)),
	motel_retractall(succ(Type,Env,MS,_,CR)),
	motel_retractall(sub(Type,Env,MS,CR,_)),
	motel_retractall(sub(Type,Env,MS,_,CR)),
	motel_retractall(nsub(Type,Env,MS,CR,_)),
	motel_retractall(nsub(Type,Env,MS,_,CR)),
	!.
...
delete_conceptName(EnvName,MS,C) :-
	environment(EnvName,Env,_),
	motel_retractall(conceptName(Env,MS,_W,C)),
	!.
...
delete_roleName(EnvName,MS,R) :-
	environment(EnvName,Env,_),
	motel_retractall(roleName(Env,MS,_W,R)),
	!.
/**********************************************************************
 *
 * @(#) sb2.pl 1.20@(#)
...
% TODO-DM	constructMLHead(Env,_RN1,W1,C0,CON,A1,X,Pc,_CALLS,_D,Exp,Gd,Od,L1,L2,Anc1,Anc2,Y,PT,InHead),
...
%TODO-DM	constructEqHead(Env,_RN1,W1,Y,CON,A1,X,Pc,_CALLS,_D,Exp,Gd,Od,L1,L2,Anc1,Anc2,Y,PT,EqHead),
...
assertRoleRInLRestr1(Env,MS,R1,restr(R2,C),AN) :-
	...
	asserta((EqLiteral1 :- (cCS(CALLS,EqMark1), (call(G1), (EqLiteral2, (ground(Y,true), onceOrMore(InHead))))))).
...
initialize :-
	retractCompiledPredicates(_),
	motel_retractall(_,axiom/3),
	motel_retractall(_,in/9),
	motel_retractall(_,kb_in/10),
	motel_retractall(_,eq/9),
	motel_retractall(_,constraint/8),
	motel_retractall(_,rel/5),
	motel_retractall(_,closed/5),
	motel_retractall(_,compiledPredicate/2),
	motel_retractall(_,conceptElement/7),
	motel_retractall(_,conceptEqualSets/6),
	motel_retractall(_,conceptHierarchy/3),
	motel_retractall(_,conceptName/4),
	motel_retractall(_,conceptSubsets/6),
	motel_retractall(_,environment/3),
	motel_retractall(_,given_change/4),
	motel_retractall(_,given_inflLink/4),
	motel_retractall(_,modalAxioms/7),
	motel_retractall(_,roleAttributes/5),
	motel_retractall(_,roleDefault/4),
	motel_retractall(_,roleDefNr/4),
	motel_retractall(_,roleDomain/4),
	motel_retractall(_,roleElement/8),
	motel_retractall(_,roleEqualSets/6),
	motel_retractall(_,roleHierarchy/3),
	motel_retractall(_,roleName/4),
	motel_retractall(_,roleNr/4),
	motel_retractall(_,roleRange/4),
	motel_retractall(_,roleSubsets/6),
	motel_retractall(_,sub/4),
	motel_retractall(_,succ/4),
	motel_retractall(_,abductiveDerivation/3),
	motel_retractall(_,consistencyDerivation/3),
	motel_retractall(_,hypothesis/1),
	motel_retractall(_,inconsistencyCheck/3),
	motel_retractall(_,option/2),
	motel_retractall(sub(_,_,_,_,_)),
	motel_retractall(nsub(_,_,_,_,_)),
	motel_retractall(succ(_,_,_,_,_)),
	motel_retractall(_,sub3/2),
	motel_retractall(_,nsub3/2),
	motel_retractall(_,succ3/2),
	motel_retractall(_,value/2),
	motel_retractall(_,query/6),
...
abduce(EnvName,MS,Hyps,elementOf(X,C),Exp) :-
	...
	call((call(G1), InHead)),
	ifOption(traceOutput,yes,(print('Hypotheses '), print(D), nl)),
...
:-setOption(allProofs,yes).
:-setOption(traceOutput,yes).
%:-setOption(prover,setheo).
:-setOption(prover,decider),
      setOption(translationMode,flatRelational),
      setOption(decider,on).
:-example(1).
...
ti:-
  makeEnvironment('cyc','cycDEmo'),
  initEnvironment,
   saved(Data,Mt,Vars,Lits),
   assertCycL(Mt,Data),
   fail.

assertCycL(_,'#$genlMt'(C,S)):-!.
assertCycL('#$BaseKB',Data):-!,assertCycLCtx([b(believe,all)],Data),!.
assertCycL('#$UniversalVocabularyMt',Data):-!,assertCycLCtx([b(believe,all)],Data),!.
assertCycL(Mt,Data):-!,assertCycLCtx([b(believe,Mt)],Data),!.

:-index(assertCycLCtx(0,1)).

assertCycLCtx(Ctx,'#$isa'(E,'#$Thing')):-!.
assertCycLCtx(Ctx,'#$isa'('#$Thing',_)):-!.
assertCycLCtx(Ctx,'#$isa'(E,C)):-!,assert_ind(Ctx,E,C).
assertCycLCtx(Ctx,'#$genls'(C,S)):-!,defprimconcept(Ctx,C,S).
assertCycLCtx(Ctx,'#$genlPreds'(C,S)):-!,defprimrole(Ctx,C,S).
assertCycLCtx(Ctx,'#$disjointWith'(C,S)):-!,sb_disjoint(Ctx,C,S).
%assertCycLCtx(Ctx,G):-compound(G),G=..[P,X,Y],!,assert_ind(Ctx,X,Y,P).
assertCycLCtx(_,_):-!.

t(E):-example(E),testMotelExample(E).
/*	
	sb_primconcept(person),
	sb_primconcept(woman,[supers([person])]),
	sb_primconcept(man,[supers([person])]),
