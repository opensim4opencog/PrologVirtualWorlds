...
write_kb_combobox(Format):-
		getMooOption(opt_kb='PrologMOO',UKB),
		%setof(KB,mooCache(PredR,_,_,_,_,KB,_),KBs),
		toMarkUp(html,select(opt_kb,[UKB|KBs]),Vars,Out),writeFmt(Format,[Out]).
		
write_ctx_combobox(Format):-
		getMooOption(opt_ctx_assert='GlobalContext',UCTX),
		%setof(CTX,mooCache(PredR,_,_,_,_,CTX,_),CTXs),
